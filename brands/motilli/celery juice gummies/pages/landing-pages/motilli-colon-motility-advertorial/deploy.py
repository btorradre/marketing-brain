#!/usr/bin/env python3
"""Deploy the Motilli colon-motility advertorial to Shopify (LIVE getmotilli.com).

Uploads all referenced assets (jpg images + the split-screen hero mp4/gif) to
Shopify Files, rewrites local `images/<file>` refs to their CDN URLs, then
creates/updates the page under template_suffix=adv-wo-v3 (chrome-suppressed).
"""
...
PAGE_HANDLE = "colon-move-on-its-own"
PAGE_TITLE = ("Top Gastroenterologist: The Common Vegetable Extract That Helps Your "
              "Colon Move On Its Own Again — Without Another Laxative")
...
ASSETS = [
    ("motilli_adv_split_hero.mp4", "file"),
    ("motilli_adv_split_hero.gif", "file"),
    ("motilli_adv_split_hero_poster.jpg", "image"),
    ("motilli_adv_byline_portrait.jpg", "image"),
    ("motilli_adv_miralax_fail.jpg", "image"),
    ("motilli_adv_bacteria_diagram.jpg", "image"),
    ("motilli_adv_celery.jpg", "image"),
    ("motilli_adv_relief_scene.jpg", "image"),
    ("motilli_adv_product.jpg", "image"),
    ("motilli_adv_testi_nancy.jpg", "image"),
    ("motilli_adv_testi_ray.jpg", "image"),
    ("motilli_adv_testi_diane.jpg", "image"),
]
...
def upload_asset(token, path: Path, kind: str) -> str:
    ...
    for _ in range(120):
        ...
        if url:
            return url.split("?")[0] if False else url
        if status == "FAILED":
