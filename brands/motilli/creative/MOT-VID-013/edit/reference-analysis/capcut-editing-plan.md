...
**Production resumed September 8, 2026.** The user now requests the actual edit. Execute the saved CapCut plan with the selected science-v2 clips and exact 291-word script, keeping three interchangeable hooks. The reference breakdown is in `reference-breakdown.md` and the 33-shot map in `reference-cut-map.json`.

### Execution addendum

Use the existing reviewed ElevenLabs take 4 as explicitly identified working narration: Woman Over 40, eleven_v3, 115.44 seconds at native speed. Preserve pitch at 1.22× in the edit, giving 94.623 seconds of narration and a 96.633-second picture with end hold. The existing word alignment is valid for this working take; review the paced export before delivery. Reference cloning remains unresolved and is not represented as completed. If that voice changes later, regenerate alignment and revise cuts/captions.

Use native CapCut materials, text and audio tracks. The installed Horizontal Blur preset was located in an existing native draft and its effect files exist; use seven frames (0.233 seconds) at the four planned boundaries, attached to outgoing clips as CapCut expects. Rechunk captions into short natural phrases, separate ingredient labels and staged guarantee/CTA. Use the existing Motilli music-library asset as a quiet working underscore, with source recorded rather than inventing licensing provenance. No new visual generation is required.
...
### Render review adjustment

The first actual CapCut render exposed a retained S06 reaction shot behind the remedies. Replace that single 3.767-second scene with four native crop inserts centered on the named bottles at frames 310, 352, 364, and 392; rejoin S07 at frame 423. This preserves source bytes, narration and full runtime. There are now 40 picture segments across the 37 narrative beats. Caption typography increased to readable short one/two-line phrases, and the last caption ends when the final CTA starts to prevent overlap.

S15 also retained a wide presenter/pen view. For the final cut, reuse the approved S36 object-only capped-pen source from 0.60 seconds, with a modest native 1.15× crop. This matches “quit your shot” and confines full-person footage to the imagined-life close. No source media is regenerated or altered.
