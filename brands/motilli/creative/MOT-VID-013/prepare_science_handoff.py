...
EDIT=HERE/'output/science-v2/editor';EDIT.mkdir(exist_ok=True)
SPEED=1.22
...
def main():
 ...
 for s in plan['shots']:
  ...
  science=HERE/'output/science-v2/motion'/f'{sid}.mp4'
  if science.exists():source=science
  science_revision=HERE/'output/science-v2/motion/revisions'/f'{sid}.mp4'
  if sid in ['S12','S22'] and science_revision.exists():source=science_revision
 ...
 (EDIT/'handoff-status.json').write_text(json.dumps({'status':'science_revision_ready_for_editor_playback','editor':'@adengine/timeline assembleFromBoard','selected_native_clips':len(picks),'narration':vo,'variants':variants},indent=2))
 (HERE/'science-v2-motion-picks.json').write_text(json.dumps(picks,indent=2))
