...
ROOT = HERE.parents[3]
SLUG = 'mot-vid-013-science-v2'
OUT = HERE / 'output/imagegen/gpt-image-2'
PROXIES = HERE / 'output/science-v2/storyboard-proxies'
...
def selected(sid):
    science = HERE / 'output/science-v2/keyframes' / (sid + '.png')
    if science.exists(): return science
...
def main():
    spec = json.loads((HERE / 'storyboard-science-v2-spec.json').read_text())
    ...
    timing={}
    for v in ['A','B','C']:
        timing.update({c['id']:c for c in json.loads((HERE/'output/editor'/f'MOT-VID-013-{v}-board.json').read_text())['lanes'][0]['cards']})
    ...
    for tl in spec['timelines']:
        ...
        for n, beat in enumerate(tl['beats']):
            if reference:
                ...
                continue
            sid = beat['id']
            beat['t']=sid+' · '+tc(timing[sid]['t'])+'–'+tc(timing[sid]['t_end'])
            beat['frame'] = preview(selected(sid), sid)
            beat['status'] = 'generated_for_review'
            beat.setdefault('keyframe_prompt', prompt_by_id[sid]['prompt'])
            ...
            beat['note'] += ' GPT Image 2 keyframe. Google Omni motion and aligned editor drafts are provided in the companion motion review.'
    ...
    for note in spec['notes']:
        ...
        if note['title'] == 'STATUS / TIMING':
            note['text'] = ('41 scene keyframes generated with GPT Image 2. Three interchangeable hooks, shared body '
                            'and future-pacing close. 96.63s per version, aligned to the selected narration.')
    ...
    spec['status']='science_revision_ready_for_review'
    for note in spec['notes']:
        if note['title']=='ASSEMBLY — CHOOSE ONE': note['text']='Choose hook A, B or C (00:00–00:04.5), then shared body and close through 01:36.6. Exact narration is identical in all variants.'
        if note['title']=='NEXT PRODUCTION STEP': note['text']='Review the source motion gallery and internal-editor drafts. Final continuous playback and export remain pending.'
    (HERE / 'storyboard-science-v2-spec.json').write_text(json.dumps(spec, ensure_ascii=False, indent=2))
