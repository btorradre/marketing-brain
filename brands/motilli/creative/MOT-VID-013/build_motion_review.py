...
def main():
 soup=BeautifulSoup((HERE/'MOT-VID-013-visual-storyboard.html').read_text(),'html.parser')
 timed={}
 for variant in ['A','B','C']:
  board=json.loads((HERE/'output/editor'/f'MOT-VID-013-{variant}-board.json').read_text())
 ...
 soup.h1.clear();soup.h1.append('Motion and narration.');soup.h1.append(soup.new_tag('br'));soup.h1.append('Three versions in progress.')
 soup.header.find('p').string='40 Google Omni clips, selected narration, and three editable timeline drafts. Review each scene below. The fiber-dish shot S22 remains unresolved after a provider rejection; final playback and export await the internal editor.'
 soup.select_one('.stats').clear()
 for value in ['40 motion clips','Google Omni','9:16 · 720p sources','~97s timeline drafts']:
 ...
 for i,v in enumerate(['A','B','C']):
  filename=f'MOT-VID-013-{v}-timeline.json';shutil.copy2(HERE/'output/editor'/filename,ASSETS/filename)
 ...
 soup.select_one('.footer-inner').string='Native source clips are shown individually. These are not final exported ads. Timeline drafts contain the exact narration, timed captions, callouts and the planned end card; S22 remains a marked gap. Source-reference frames below are context only.'
 ...
 (HERE/'MOT-VID-013-motion-review.html').write_text(page)
