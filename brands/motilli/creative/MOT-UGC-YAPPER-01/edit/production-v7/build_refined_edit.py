...
O=Path(__file__).resolve().parent;V=O.parent/'production-v6';meta=json.loads((O/'final-speech-ranges.json').read_text());rows=[]
...
for segment in meta['ranges']:
 row('V3 Creative final speech range',O/'narration-mix.wav',segment['record_start'],segment['record_end'],segment['source_start'],kind='audio',props={'AudioVolumeEnabled':True,'AudioVolume':0.0})
for ins in json.loads((O/'aligned-inserts.json').read_text()):
 props={}
 if ins['type']=='broll':path=V/'normalized'/(ins['asset']+'-30fps.mp4');source=30
 else:
  path=O/'graphics-lock'/(ins['asset']+'.mov');source=0;w,h=Image.open(ins['source_image']).size;fitted=min(1920,1080*h/w);zoom=min(.74,400/fitted);props={'ZoomX':zoom,'ZoomY':zoom,'Tilt':-200}
...
code='''local r=fu:GetResolve();local pm=r:GetProjectManager();local p=pm:GetCurrentProject();assert(p:GetName()=='MOT-UGC-YAPPER-01 v7 V3 20260910');assert(not p:IsRenderingInProgress());for i=1,p:GetTimelineCount() do assert(p:GetTimelineByIndex(i):GetName()~='v7 V3 Creative - final pacing edit','Edit exists; inspect') end
local mp=p:GetMediaPool();local t=mp:CreateEmptyTimeline('v7 V3 Creative - final pacing edit');assert(t);p:SetCurrentTimeline(t);t:SetStartTimecode('00:00:00:00');t:AddTrack('video');t:AddTrack('video');t:SetTrackName('video',1,'PENDING fresh HeyGen Avatar V');t:SetTrackName('video',2,'20 distinct covering scenes');t:SetTrackName('video',3,'V3 aligned dialogue captions');t:SetTrackName('audio',1,'ElevenLabs V3 Creative - pauses removed');local rows=__ROWS__;local media={};local bad=0
for _,row in ipairs(rows) do if not media[row.path] then media[row.path]=mp:ImportMedia({row.path})[1] end;local clip=mp:AppendToTimeline({{mediaPoolItem=media[row.path],startFrame=row.source,endFrame=row.source+row['end']-row.start,recordFrame=row.start,trackIndex=row.track,mediaType=row.kind=='audio' and 2 or 1}})[1];assert(clip,'Append '..row.name);if row.kind=='audio' then assert(clip:SetFades({FadeIn=1,FadeOut=1})) end;if next(row.props) then assert(clip:SetProperty(row.props),'Props '..row.name) end;if clip:GetStart()~=row.start or clip:GetEnd()~=row['end'] then print('RANGE_MISMATCH',row.name,clip:GetStart(),clip:GetEnd());bad=bad+1 end end
...
(O/'assemble-refined-edit.lua').write_text(code);print('Prepared',len(rows),'native timeline rows')
