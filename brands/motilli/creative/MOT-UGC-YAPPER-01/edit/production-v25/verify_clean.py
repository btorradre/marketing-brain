...
for h in sys.argv[1:] or ['H1','H2','H3']:
 d=json.loads((O/f'{h}-pause-edit.json').read_text());p=O/f'{h}-clean-master-resolve.mov'
 def pcm(f):
 ...
 dest=O/f'deliverables/Motilli-V25-{h}-Woman-Over-40-Natural-1.2x-clean.mp3';subprocess.run(['ffmpeg','-v','error','-y','-i',str(p),'-vn','-af','pan=mono|c0=0.5*c0+0.5*c1','-ar','44100','-codec:a','libmp3lame','-b:a','192k',str(dest)],check=True)
 wav=O/f'deliverables/Motilli-V25-{h}-Woman-Over-40-Natural-1.2x-clean.wav';subprocess.run(['ffmpeg','-v','error','-y','-i',str(p),'-vn','-af','pan=mono|c0=0.5*c0+0.5*c1','-c:a','pcm_s24le',str(wav)],check=True)
