...
ROOT=pathlib.Path('/Users/brooksorradre2/Documents/marketing brain');P=ROOT/'brands/motilli/creative/MOT-UGC-YAPPER-01';O=P/'edit/production-v18';D=P/'assets/video-v18';D.mkdir(exist_ok=True)
...
with httpx.Client(timeout=180,headers={'X-Api-Key':key}) as c:
 ...
 if (O/'heygen-video-job.json').exists():job=json.loads((O/'heygen-video-job.json').read_text())
  ...
  payload={'type':'avatar','avatar_id':look,'audio_asset_id':aid,'engine':{'type':'avatar_v'},'aspect_ratio':'9:16','resolution':'1080p','title':'Motilli Yapper v18 Michelle age50 Eleven v3 Creative','motion_prompt':'A mature American woman speaks candidly to a friend from her parked car. Natural restrained expressions, small head movements and occasional gentle nod. Keep the same identity, clothing, background and fixed framing throughout. Hands relaxed and empty. No products, logos, props, large gestures or camera movements.'}
