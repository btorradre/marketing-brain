...
O=Path(__file__).resolve().parent;P=O.parents[1];src=P/'assets/video-v18/presenter-avatar-v.mp4';N=O/'normalized';Q=O/'qa';N.mkdir(exist_ok=True);Q.mkdir(exist_ok=True);expected=json.loads((O/'pause-edit-spec.json').read_text())['frames'];assert src.exists()
...
subprocess.run(['ffmpeg','-v','error','-y','-ss','0.5','-i',str(out),'-frames:v','1',str(Q/'selected-presenter-v18.png')],check=True)
