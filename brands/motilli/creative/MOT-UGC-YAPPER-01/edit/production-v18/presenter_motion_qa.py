...
O=pathlib.Path(__file__).resolve().parent;P=O.parents[1];root=O.parents[5];Q=O/'qa';v=P/'assets/video-v18/presenter-avatar-v.mp4';key=next(l.split('=',1)[1].strip().strip('"').strip("'") for l in (root/'.env').read_text().splitlines() if l.startswith('GEMINI_API_KEY='))
def check(t):
...
with concurrent.futures.ThreadPoolExecutor(max_workers=3) as ex:list(ex.map(check,[5,150,299]))
