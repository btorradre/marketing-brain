...
O=Path(__file__).resolve().parent;Q=O/'qa';Q.mkdir(exist_ok=True);V=O/'deliverables/Motilli-Unbranded-VSL-v17.mp4';old=O.parent/'production-v16/deliverables/Motilli-Unbranded-VSL-v16.mp4'
...
report={'duration_seconds':v['duration'],'frames':8327,'resolution':[1080,1920],'fps':30,'audio_correlation_to_v16':corr,'max_pcm_difference':err,'replacement_frames':[[1986,2044]],'review_frames':frames,'unchanged_audio_verified':True};(Q/'revision-qa.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2),flush=True)
