...
for h in sys.argv[1:] or ['H1','H2','H3']:
 d=json.loads((O/f'{h}-clean-edit.json').read_text());p=O/f'{h}-final120-master.mov';x=np.frombuffer(subprocess.check_output(['ffmpeg','-v','error','-i',str(p),'-af','pan=mono|c0=0.5*c0+0.5*c1','-ar','48000','-f','f32le','-']),np.float32)
