...
parts=[]
for label,p,start in [('A',O.parent/'production-v18/narration.mp3',10),('B',O/'deliverables/Motilli-V23-H1-Eleven-v3-Natural-1.2x.mp3',0)]:
...
j=r.json();z=''.join(t.get('text','') for a in j['candidates'] for t in a['content']['parts']);(O/'qa/voice-comparison.json').write_text(z);print(z,flush=True)
