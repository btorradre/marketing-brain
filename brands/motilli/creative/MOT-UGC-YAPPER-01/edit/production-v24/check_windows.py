...
def work(pair):
 h,t=pair;f=O/f'qa/{h}-window-{t}.mp3';subprocess.run(['ffmpeg','-v','error','-y','-ss',str(t),'-i',str(O/f'deliverables/Motilli-V24-{h}-Natural-1.2x-clean.mp3'),'-t','12',str(f)],check=True)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ex:list(ex.map(work,[(h,t) for h in ['H1','H2'] for t in [26,86,206]]))
