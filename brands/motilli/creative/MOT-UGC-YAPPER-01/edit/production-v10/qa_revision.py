...
O=Path(__file__).resolve().parent;Q=O/'qa';Q.mkdir(exist_ok=True);V=O/'deliverables/Motilli-Unbranded-VSL-v10.mp4';old=O.parent/'production-v9/deliverables/Motilli-Unbranded-VSL-v9.mp4'
...
frames=[4196,4197,4205,4257,4316,4317,4618,4619,4634,4685,4750,4751]
for f in frames:subprocess.run(['ffmpeg','-v','error','-y','-ss',f'{f/30:.9f}','-i',str(V),'-frames:v','1','-q:v','2',str(Q/f'frame-{f}.jpg')],check=True)
im=Image.new('RGB',(1200,1480),'#eee');d=ImageDraw.Draw(im)
...
report={'duration_seconds':v['duration'],'frames':8327,'resolution':[1080,1920],'fps':30,'audio_correlation_to_v9':corr,'max_pcm_difference':err,'replacement_frames':[4619,4751],'review_frames':frames,'unchanged_audio_verified':True};(Q/'revision-qa.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2),flush=True)
