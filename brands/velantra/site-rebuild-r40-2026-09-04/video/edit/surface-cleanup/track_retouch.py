...
def process(video, clean_path, start, end, anchor_index, output, extra_references=None, surface_only=False, plate_only=False, smooth_planar=False, polish_surface=False, preserve_clean_lighting=False):
    ...
    # Newly restaged authentic product photographs have independent lighting
    # and framing. Never copy the damaged video's shadows/texture onto them.
    plate = cv2.resize(clean, (anchor.shape[1], anchor.shape[0]), interpolation=cv2.INTER_LANCZOS4) if preserve_clean_lighting else align_clean(anchor, clean)
    ...
    for index in range(start,end):
        ...
        for ref_index,old,ref_plate in references:
            ...
            if surface_only:
                confidence[:]=1
                if not plate_only or preserve_clean_lighting:valid[:]=1
        ...
        if polish_surface:
            ...
            result=cv2.bilateralFilter(result,9,14,4)
    ...
    receipt={'source':str(video),'generated_reference':str(clean_path),'start_frame':start,
        ...
        'samples':samples,'surface_polish':polish_surface,'preserve_clean_lighting':preserve_clean_lighting,'status':'awaiting_visual_review'}
...
if __name__=='__main__':
    ...
    p.add_argument('--preserve-clean-lighting',action='store_true')
    a=p.parse_args();process(a.video,a.clean,a.start,a.end,a.anchor,a.output,[(int(x.split('=',1)[0]),x.split('=',1)[1]) for x in a.extra],a.surface_only,a.plate_only,a.smooth_planar,a.polish_surface,a.preserve_clean_lighting)
