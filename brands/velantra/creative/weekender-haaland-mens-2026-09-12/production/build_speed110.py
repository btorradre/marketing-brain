...
local src;for i=1,p:GetTimelineCount() do local t=p:GetTimelineByIndex(i);assert(t:GetName()~="Weekender-Haaland-Gringo-PausesClosed-v1");assert(t:GetName()~="Weekender-Haaland-Gringo-Natural-AvatarV-110-v1");if t:GetName()=="Weekender-Haaland-Gringo-Natural-AvatarV-v2" then src=t end end;assert(src and src:GetEndFrame()==1431)
local pool=p:GetMediaPool();local t=pool:CreateEmptyTimeline("Weekender-Haaland-Gringo-PausesClosed-v1");assert(t);assert(p:SetCurrentTimeline(t));assert(t:SetStartTimecode("00:00:00:00"))
...
assert(t:GetEndFrame()==1145)
local final=pool:CreateEmptyTimeline("Weekender-Haaland-Gringo-Natural-AvatarV-110-v1");assert(final);assert(p:SetCurrentTimeline(final));assert(final:SetStartTimecode("00:00:00:00"));assert(pool:AppendToTimeline({{mediaPoolItem=t:GetMediaPoolItem(),startFrame=0,endFrame=1145,recordFrame=0}}))
local v=final:GetItemListInTrack("video",1)[1];local a=final:GetItemListInTrack("audio",1)[1];assert(v and a);assert(final:SetClipsLinked({v,a},true));assert(v:SetSpeed({Percentage=110,PitchCorrection=true,RippleTimeline=true}));assert(v:GetSpeed().Percentage==110 and a:GetSpeed().Percentage==110);assert(v:GetEnd()==a:GetEnd());assert(v:GetSpeed().PitchCorrection and a:GetSpeed().PitchCorrection)
assert(pm:SaveProject());assert(final:Export(output.."/Weekender-Haaland-Gringo-Natural-AvatarV-110-v1.drt",r.EXPORT_DRT));assert(t:Export(output.."/Weekender-Haaland-Gringo-PausesClosed-v1.drt",r.EXPORT_DRT));assert(pm:ExportProject(p:GetName(),output.."/VEL-Weekender-Haaland-Gringo-Speed110.drp",true));r:OpenPage("edit")
...
o=run(s,30);(R/'build-receipt.json').write_text(json.dumps(o,indent=2));print(o);assert o['ok'],o
