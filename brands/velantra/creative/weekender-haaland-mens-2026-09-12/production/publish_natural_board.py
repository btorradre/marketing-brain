...
v=P/'exports/Weekender-Haaland-Natural-AvatarV.mp4';S=json.loads((P/'resolve-natural/aligned-scenes.json').read_text());spec=json.loads((B/'storyboard/spec.json').read_text());remote=json.loads(cloud.download(f'boards/{slug}.json'));before=copy.deepcopy(remote);(B/'storyboard/board-before-natural-publish.json').write_text(json.dumps(before,indent=2))
A=root/'cutroom/assets'/slug;F=P/'qa-natural/final-frames';F.mkdir(exist_ok=True);receipt=[]
visual_updates={'S04':'Airport outfit with the Cognac Weekender; restrained step and head turn. Correct paired front straps and rolled handles.','S06':'Distinct white T-shirt and jeans outfit still with corrected Cognac Weekender. Omni generation failed; inspected still selected.','S07':'Moving close-up of Cognac leather, light canvas and gold-tone hardware; hand lightly traces leather.','S11':'Actual owner-shot footage of the Cognac Weekender opening and its wide slip pocket. Original phone clip IMG_4051.MOV,8.50–13.33s; no generated zipper.','S12':'Hotel entrance outfit with the Cognac Weekender; restrained settling step and head turn.'}
for i,s in enumerate(S):
 sid=s['id'];fn=f'{sid}-natural-avatarv.jpg';f=A/fn
...
concept='45.70-second Resolve ad, aligned to the selected permitted reference-voice clone. Original HeyGen presenter, Google Omni motion and distinct corrected wardrobe stills. Exact approved narration preserved. These 13 images are actual first frames from the Natural / Avatar V export; the separate reference lane remains source evidence.'
for c in remote['cards']:
 if c.get('title')=='CONCEPT':c['text']=concept
 if c.get('title','').startswith('VOICE'):
  c['title']='VOICE — SELECTED / HEYGEN';c['text']='User confirmed speaker permission and lawful use in this conversation. Fresh permitted reference clone; Eleven v3 Natural0.5 drives the original HeyGen presenter. Narration remains at original speed; final export audio has zero measured offset. Three rendered lip-sync windows checked.'
 if c.get('title')=='AUDIO / ALIGNMENT':c['text']='Selected original-speed narration drives all cuts and 31 phrase captions. Speech-only mix, no copied reference music. Final clean hero ends at45.70seconds after the spoken CTA and a two-second hold.'
 if c.get('type')=='label' and 'PROPOSED AD / SELECTED FIRST FRAMES' in c.get('text',''):c['text']='VELANTRA — RENDERED AD / ACTUAL FIRST FRAMES'
spec['summary']=concept;spec['timelines'][0]['label']='VELANTRA — RENDERED AD / ACTUAL FIRST FRAMES';spec['timelines'][0]['source']='Resolve Natural / Avatar V · original HeyGen presenter · fresh permitted reference clone / Eleven v3 Natural · GPT Image2 / Google Omni · voice aligned'
for n in spec['notes']:
 for c in remote['cards']:
...
out={'published_at':datetime.now(timezone.utc).isoformat(),'board_url':'https://cutroom-three.vercel.app/b/'+slug,'export':str(v),'timeline':'Weekender-Haaland-Natural-AvatarV-v2','images':receipt};(B/'storyboard/natural-board-receipt.json').write_text(json.dumps(out,indent=2));print({'board_url':out['board_url'],'frames':len(receipt)})
