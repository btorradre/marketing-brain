...
P=Path(__file__).resolve().parent;R=P/'resolve-speed110';Q=P/'qa-speed110';Q.mkdir(exist_ok=True);F=Q/'frames';F.mkdir(exist_ok=True)
v=P/'exports/Weekender-Haaland-Gringo-Natural-AvatarV-110-NoGaps.mp4';old=P/'exports/Weekender-Haaland-Gringo-Natural-AvatarV-Final.mp4';m=json.loads((R/'time-map-plan.json').read_text());K=m['keep']
...
a=raw(v);b=raw(old);assert len(a)==1040
...
def mapped(f):return sum(max(0,min(f,k['source_end'])-k['source_start']) for k in K)/1.1
...
for i,k in enumerate(K[1:]):
 f=math.ceil(k['record_start']/1.1);bounds[f]=bounds.get(f,'')+f' pause{i+1}'
...
frame(1039).save(Q/'last-frame.jpg')
