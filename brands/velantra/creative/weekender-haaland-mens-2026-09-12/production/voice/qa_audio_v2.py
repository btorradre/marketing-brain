...
r=c.models.generate_content(model='gemini-2.5-pro',contents=[prompt,types.Part.from_bytes(data=(v/'reference-source.wav').read_bytes(),mime_type='audio/wav'),types.Part.from_bytes(data=(v/'narration-v2-creative.mp3').read_bytes(),mime_type='audio/mpeg')],config=types.GenerateContentConfig(response_mime_type='application/json'))
(v/'audio-qa-v2-model.json').write_text(r.text+'\n');print(r.text)
