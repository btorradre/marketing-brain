...
v=P/'exports/Weekender-Haaland-Gringo-Natural-AvatarV.mp4';S=json.loads((P/'resolve-gringo/aligned-scenes.json').read_text());spec=json.loads((B/'storyboard/spec.json').read_text());remote=json.loads(cloud.download(f'boards/{slug}.json'));before=copy.deepcopy(remote);(B/'storyboard/board-before-gringo-publish.json').write_text(json.dumps(before,indent=2))
A=root/'cutroom/assets'/slug;F=P/'qa-gringo/final-frames';F.mkdir(exist_ok=True);receipt=[]
...
for i,s in enumerate(S):
 sid=s['id'];fn=f'{sid}-gringo-natural-avatarv.jpg';f=A/fn
...
out={'published_at':datetime.now(timezone.utc).isoformat(),'board_url':'https://cutroom-three.vercel.app/b/'+slug,'export':str(v),'timeline':'Weekender-Haaland-Gringo-Natural-AvatarV-v1','images':receipt};(B/'storyboard/gringo-board-receipt.json').write_text(json.dumps(out,indent=2));print({'board_url':out['board_url'],'frames':len(receipt)})
