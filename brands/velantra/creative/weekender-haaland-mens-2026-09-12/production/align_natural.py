...
import json,re,math
P=Path(__file__).resolve().parent;B=P.parent;V=P/'voice-natural-corrected';R=P/'resolve-natural';R.mkdir(exist_ok=True)
...
text=''.join(chars);assert text==(B/'storyboard/narration.txt').read_text().strip();words=[]
...
scenes[0]['start']=0
for i,s in enumerate(scenes):s['end']=scenes[i+1]['start'] if i+1<len(scenes) else math.ceil(words[-1]['end']*30)+60
caps=[]
for s in scenes[:-1]:
