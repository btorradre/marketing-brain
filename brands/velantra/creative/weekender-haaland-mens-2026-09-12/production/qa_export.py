...
P=Path(__file__).resolve().parent;Q=P/'qa';root=P.parents[4];v=P/'exports/Weekender-Haaland-review-v6.mp4'
assert v.exists()
...
a=audio(P/'voice/narration-v2-creative.mp3');b=audio(v);c=correlate(b,a,method='fft');lag=int(correlation_lags(len(b),len(a))[np.argmax(c)]);n=min(len(a),len(b)-max(0,lag));corr=float(np.corrcoef(a[:n],b[max(0,lag):max(0,lag)+n])[0,1]);(Q/'export-audio-sync.json').write_text(json.dumps({'lag_samples_8000hz':lag,'lag_ms':lag/8,'correlation':corr,'decode_errors':r.stderr},indent=2));print('audio lag ms',lag/8,'correlation',corr,flush=True)
# Three lip windows from the actual final export, at original playback speed.
parts=['These clips are from the finished Velantra ad. Check the presenter inset mouth against actual speech at 24fps. Inspect opening 0–3s (inset changes left to right and back), middle 17–20s and end 35–38s (left inset). Report concrete phoneme observations, any sustained lead/lag, whether captions match speech when present, visual artifacts, and acceptable_for_review. Presenter is synthetic; that alone is not a defect. Do not claim exhaustive human listening. Return JSON.']
for label,start in [('opening',0),('middle',17),('end',35)]:
