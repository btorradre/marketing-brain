...
P=Path(__file__).resolve().parent;R=P/'heygen-natural';R.mkdir(exist_ok=True)
...
if mode=='prepare':
 for kind,path in [('image',P/'plates/presenter-clean.png'),('audio',P/'voice-natural/narration-natural.mp3')]:
 ...
 payload={'type':'avatar','avatar_id':state['look_id'],'audio_asset_id':state['audio_asset_id'],'engine':{'type':'avatar_v'},'resolution':'1080p','aspect_ratio':'auto','title':'Weekender — Fresh Reference Clone — Eleven v3 Natural — Avatar V','motion_prompt':'Natural conversational delivery to camera, relaxed but engaged expression, subtle varied eyebrow emphasis and small free-hand gestures. Keep the handheld microphone steady, clear mouth articulation and consistent face identity. No exaggerated head bobbing or repetitive swaying.'}
