...
P=Path(__file__).resolve().parent;R=P/'resolve-gringo-aligned';Q=P/'qa-gringo-aligned';Q.mkdir(exist_ok=True);v=P/'exports/Weekender-Haaland-Gringo-Natural-AvatarV-Final.mp4';M=json.loads((R/'manifest.json').read_text());S=M['scenes'];C=M['captions'];F=Q/'frames';F.mkdir(exist_ok=True)
def frame(n):
 p=F/f'{n:05d}.png'
...
a=audio(P/'voice-gringo-natural/narration-natural.mp3');b=audio(v);corr=correlate(b,a,method='fft');lag=int(correlation_lags(len(b),len(a))[np.argmax(corr)]);audioqa={'lag_ms':lag/8,'correlation':float(np.corrcoef(a,b[:len(a)])[0,1]),'speech_end':json.loads((P/'voice-gringo-natural/words.json').read_text())[-1]['end'],'picture_end':M['frames']/30};(Q/'audio-sync.json').write_text(json.dumps(audioqa,indent=2))
