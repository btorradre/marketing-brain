...
P=Path(__file__).resolve().parent;B=P.parent;R=P/'resolve-gringo-aligned';FPS=30
S=json.loads((R/'aligned-scenes.json').read_text());caps=json.loads((P/'voice-gringo-natural/captions.json').read_text());end=S[-1]['end'];R.mkdir(exist_ok=True);(R/'comps').mkdir(exist_ok=True)
...
subprocess.run(['ffmpeg','-v','error','-y','-i',str(P/'voice-gringo-natural/narration-natural.mp3'),'-c:a','pcm_s24le','-ar','48000',str(audio)],check=True)
...
def clip(path,n,start=0):
 ...
 return {'OTIO_SCHEMA':'Clip.2','name':path.name,'metadata':{},'source_range':tr(start+offset,n),'effects':[],'markers':[],'enabled':True,'active_media_reference_key':'DEFAULT_MEDIA','media_references':{'DEFAULT_MEDIA':{'OTIO_SCHEMA':'ExternalReference.1','name':path.name,'metadata':{},'target_url':str(path),'available_range':tr(offset,int(json.loads((R/'presenter-master-probe.json').read_text())['streams'][0]['nb_frames']) if offset else max(n+start,1800)),'available_image_bounds':None}}}
...
bg=[];meta=[];prs=[]
heygen=P/'heygen-gringo/presenter-keyed-master.mov';ready=heygen.exists();presenter=heygen if ready else P/'plates/presenter-clean.png'
for s in S:
 ...
 elif sid=='S11':path=B.parents[1]/'products/weekender/actual product assets/IMG_4051.MOV'
 elif sid in ['S04','S12']:path=P/'omni'/f'{sid}-restrained.mp4'
 elif sid=='S07':path=P/'omni/S07-native.mp4'
 else:path=M/(aliases[sid]+'.png')
 assert path.exists(),path
 source_start=230 if sid=='S11' else 0
 bg.append((s['start'],clip(path,n,source_start)));meta.append(dict(scene=sid,path=str(path),start=s['start'],duration=n,source_start=source_start))
...
for c in caps:
 c['x']=.5;c['y']=.23 if c['scene']=='S07' else (.86 if c['scene']=='S11' else .53)
 c['size']=.068
...
tracks=[track('V1 — Picture','Video',bg),track('V2 — HeyGen presenter','Video',prs),track('V3 — Phrase captions','Video',capitems),track('V4 — Hook titles','Video',hooks),track('A1 — gringo-tiktok-male / Eleven v3 Natural','Audio',[(0,clip(audio,end))])]
name='Weekender-Haaland-Gringo-Natural-AvatarV-v2' if ready else 'Weekender-Haaland-Assembly-WIP'
...
assert ready, 'Wait for actual regenerated presenter master'
assert int(json.loads((R/'presenter-master-probe.json').read_text())['streams'][0]['nb_frames'])>=S[-2]['end']
