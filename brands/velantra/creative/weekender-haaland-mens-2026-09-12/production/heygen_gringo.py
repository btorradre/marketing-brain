...
P=Path(__file__).resolve().parent;R=P/'heygen-gringo';R.mkdir(exist_ok=True)
...
statepath=R/'state.json';state=json.loads(statepath.read_text()) if statepath.exists() else {'look_id':'5e9bb28bebe834bfce06d8cbfcdf1a00'}
...
if mode=='prepare':
 for kind,path in [('image',P/'plates/presenter-clean.png'),('audio',P/'voice-gringo-natural/narration-natural.mp3')]:
 ...
 if 'look_id' not in state:
  d=api('POST','/v3/avatars',{'type':'photo','name':'Weekender Haaland Gringo Natural Avatar V','file':{'type':'asset_id','asset_id':state['image_asset_id']}})
  state['avatar_response']=d;state['look_id']=d['avatar_item']['id'];save()
 print(json.dumps({k:state[k] for k in ['audio_asset_id','look_id']}))
elif mode=='submit':
 assert 'video_id' not in state and not state.get('submission_started'),'Already submitted: inspect provider before retry'
 ...
 payload={'type':'avatar','avatar_id':state['look_id'],'audio_asset_id':state['audio_asset_id'],'engine':{'type':'avatar_v'},'resolution':'1080p','aspect_ratio':'auto','title':'Weekender — gringo-tiktok-male — Eleven v3 Natural — Avatar V'}
