...
P=Path(__file__).resolve().parent;B=P.parent;V=P/'voice-gringo-natural';R=P/'resolve-gringo-aligned';R.mkdir(exist_ok=True)
fa=json.loads((V/'forced-alignment.json').read_text());text=''.join(c['text'] for c in fa['characters']);starts=[c['start'] for c in fa['characters']];ends=[c['end'] for c in fa['characters']];chars=[];ss=[];ee=[];i=0
