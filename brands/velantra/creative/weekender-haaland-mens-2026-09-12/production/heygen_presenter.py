...
P=Path(__file__).resolve().parent;R=P/'heygen';R.mkdir(exist_ok=True)
...
if mode=='prepare':
 for kind,path in [('image',P/'plates/presenter-clean.png'),('audio',P/'voice/narration-v2-creative.mp3')]:
 ...
 if 'look_id' not in state:
  d=api('POST','/v3/avatars',{'type':'photo','name':'Weekender Haaland Male Presenter','file':{'type':'asset_id','asset_id':state['image_asset_id']}})
 ...
 payload={'type':'avatar','avatar_id':state['look_id'],'audio_asset_id':state['audio_asset_id'],'engine':{'type':'avatar_v'},'resolution':'1080p','aspect_ratio':'auto','title':'Weekender Haaland — Original Male — Permitted Reference Clone'}
