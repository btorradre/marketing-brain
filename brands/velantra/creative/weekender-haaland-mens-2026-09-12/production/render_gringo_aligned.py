...
import sys,json
P=Path(__file__).resolve().parent;R=P/'resolve-gringo-aligned';sys.path.insert(0,str(R));from run_lua import run
...
local t=p:GetCurrentTimeline() assert(t:GetName()=="Weekender-Haaland-Gringo-Natural-AvatarV-v2")
assert(p:SetCurrentRenderFormatAndCodec("mp4","H264")) assert(p:SetCurrentRenderMode(1))
assert(p:SetRenderSettings({SelectAllFrames=true,TargetDir=output,CustomName="Weekender-Haaland-Gringo-Natural-AvatarV-Final",ExportVideo=true,ExportAudio=true,FormatWidth=1080,FormatHeight=1920,FrameRate=30,VideoQuality=16000,AudioCodec="aac",AudioSampleRate=48000,NetworkOptimization=true}))
