...
P=Path(__file__).resolve().parent;B=P.parent;R=P/'resolve-natural';FPS=30
S=json.loads((R/'aligned-scenes.json').read_text());caps=json.loads((P/'voice-natural-corrected/captions.json').read_text());end=S[-1]['end'];R.mkdir(exist_ok=True);(R/'comps').mkdir(exist_ok=True)
...
subprocess.run(['ffmpeg','-v','error','-y','-i',str(P/'voice-natural-corrected/narration-natural.mp3'),'-c:a','pcm_s24le','-ar','48000',str(audio)],check=True)
...
bg=[];meta=[];prs=[]
heygen=P/'heygen-natural-corrected/presenter-keyed-master.mov';ready=heygen.exists();presenter=heygen if ready else P/'plates/presenter-clean.png'
for s in S:
 ...
 source_start=255 if sid=='S11' else 0
...
tracks=[track('V1 — Picture','Video',bg),track('V2 — HeyGen presenter','Video',prs),track('V3 — Phrase captions','Video',capitems),track('V4 — Hook titles','Video',hooks),track('A1 — Permitted reference clone','Audio',[(0,clip(audio,math.ceil(json.loads((P/'voice-natural-corrected/words.json').read_text())[-1]['end']*30)))])]
name='Weekender-Haaland-Natural-AvatarV-v2' if ready else 'Weekender-Haaland-Assembly-WIP'
