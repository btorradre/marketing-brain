...
import json,sys
P=Path(__file__).resolve().parent;R=P/'resolve-natural';sys.path.insert(0,str(R));from run_lua import run
...
local pool=p:GetMediaPool() local media=pool:ImportMedia({root.."/heygen-natural-corrected/presenter-native.mp4"}) assert(#media==1)
local t=pool:CreateEmptyTimeline("Presenter-Natural-Key-Master") assert(t) assert(p:SetCurrentTimeline(t))
local clips=pool:AppendToTimeline({{mediaPoolItem=media[1],startFrame=0,endFrame=1091,mediaType=1,trackIndex=1,recordFrame=t:GetStartFrame()}}) assert(#clips==1)
...
key:ConnectInput("Input",mi) mo:ConnectInput("Input",key) c:Unlock() local f=root.."/heygen-natural-corrected/presenter-master-key.comp" assert(item:ExportFusionComp(f,1)) assert(item:ImportFusionComp(f))
