...
P=Path(__file__).resolve().parent;Q=P/'qa-natural';root=P.parents[4];v=P/'exports/Weekender-Haaland-Natural-AvatarV.mp4'
assert v.exists()
...
parts=['These clips are from the finished Velantra ad. Check the presenter inset mouth against actual speech at 24fps. Inspect opening 0–3s (inset changes left to right and back), middle 18–21s and end 37–40s (left inset). Report concrete phoneme observations, any sustained lead/lag, whether captions match speech when present, visual artifacts, and acceptable_for_review. Presenter is synthetic; that alone is not a defect. Do not claim exhaustive human listening. Return JSON.']
for label,start in [('opening',0),('middle',18),('end',37)]:
