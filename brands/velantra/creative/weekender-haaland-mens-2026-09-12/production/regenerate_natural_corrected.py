...
P=Path(__file__).resolve().parent;B=P.parent;root=P.parents[4];V=P/'voice-natural-corrected';V.mkdir(exist_ok=True);statep=V/'state.json';state=json.loads(statep.read_text()) if statep.exists() else {'voice_id':json.loads((P/'voice-natural/state.json').read_text())['voice_id'],'pronunciation_alias':{'Velantra':'Vell-Ahn-Trah'},'permission':'Same fresh permitted reference clone; pronunciation correction only'}
...
if not (V/'narration-natural.mp3').exists():
 ...
 payload={'text':(B/'storyboard/narration.txt').read_text().strip().replace('Velantra','Vell-Ahn-Trah'),'model_id':'eleven_v3','voice_settings':{'stability':0.5,'similarity_boost':0.85,'use_speaker_boost':True}}
