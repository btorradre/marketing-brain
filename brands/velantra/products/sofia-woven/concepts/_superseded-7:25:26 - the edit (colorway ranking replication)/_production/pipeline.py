#!/usr/bin/env python3
"""VEL-SOFIA-EDIT-01 pipeline — kie.ai.

  pipeline.py prompts [K00 S01 ...]   print assembled prompts, no API calls
  pipeline.py images  [K00 ...]       GPT Image 2 i2i keyframes  (~10 cr each)
    ...
    for attempt in range(4):
        ...
        out = subprocess.run(
            ["curl", "-s", "-X", "POST", UPLOAD_URL,
             ...
             "-F", f"file=@{path}",
             "-F", "uploadPath=velantra-sofia-edit",
...
def cmd_images(ids):
    ...
    for name, cw, blocking in pick(B.KEYFRAMES, ids):
        if os.path.exists(os.path.join(ASSETS, "keyframes", f"{name}.png")):
            ...
            continue
        urls = [cam] + ([B.COLORWAYS[cw]["hero"]] if cw else [])
        payload = {"model": IMG_MODEL, "input": {
            "prompt": B.image_prompt(cw, blocking),
            "input_urls": urls,
            "aspect_ratio": "9:16",
            "resolution": "2K"}}
