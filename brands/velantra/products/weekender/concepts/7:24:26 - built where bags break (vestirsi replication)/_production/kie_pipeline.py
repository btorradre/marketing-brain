#!/usr/bin/env python3
"""VEL-WEEKENDER-BUILT-CRAFT-01 asset pipeline.
...
BANNED = ["made in italy", "italian", "italy", "made in usa", "made in the usa",
          "european", "made in europe", "birkin", "hermes", "hermès", "weekender"]
    ...
    for attempt in range(4):
        ...
        out = subprocess.run(
            ["curl", "-s", "-X", "POST", UPLOAD_URL,
             ...
             "-F", f"file=@{path}",
             "-F", "uploadPath=velantra-weekender-craft",
...
def build_image_prompt(shot):
    lock = MANIFEST["product_lock"] + " " + MANIFEST["closed_pin"] \
...
def build_video_prompt(shot):
    ...
    if shot["subject"] == "product":
        parts.append(MANIFEST["kling_closed_pin"])
...
def cmd_prompts(ids):
    for shot in shots_by_id(ids):
        print(f"===== {shot['id']} ({shot['time']}) subject={shot['subject']} =====")
