#!/usr/bin/env python3
"""Composite VEL-VIV-OLDMONEY-GRID-01 (Annalise Salm grid-hook reel mirror).
...
GRID = os.environ.get("GRID", "GRID-A-workday")
...
SCRIM_FROM = 57.0                                # cards over the light PDP page get a feathered dark scrim
...
for b in BEATS:
    if b["beat"] == "B13": SRC[b["beat"]] = "pdp/PDP-scroll.mp4"
    elif b["beat"] == "B14": SRC[b["beat"]] = None            # PDP recording spans B13+B14
...
def build(creator):
    out = os.path.join(ROOT, "final", f"VEL-VIV-OLDMONEY-GRID-01-{creator}-{GRID.split('-')[1]}.mp4")
