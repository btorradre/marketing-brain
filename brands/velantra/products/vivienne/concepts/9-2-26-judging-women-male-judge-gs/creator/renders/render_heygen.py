#!/usr/bin/env python3
"""HeyGen Avatar V render for VEL-VIV-JUDGE-01. usage: render_heygen.py G1 [G2 G3]
...
HERE = Path(__file__).resolve().parent; CRE = HERE.parent; VO = HERE.parent.parent / "vo" / ("VO-gringo-" + os.environ.get("TAKE", "J") + ".mp3")
...
H = {"X-Api-Key": KEY}
STILLS = {"G1": "G1-charcoal-green.png", "G2": "G2-black-green.png", "G3": "G3-heather-green.png"}
def upload(path):
...
names = sys.argv[1:] or ["G1"]
AK = "audio_asset_id_" + os.environ.get("TAKE", "J")
if AK not in state: state[AK] = upload(VO); save()
state["audio_asset_id"] = state[AK]; save()
...
for n in names:
    ...
    if not s.get("look_id"):
        d = post("https://api.heygen.com/v3/avatars", {"type":"photo","name":f"viv-judge-{n}","file":{"type":"asset_id","asset_id":s["image_asset_id"]}})["data"]
    ...
    if not s.get("video_id"):
        ...
        for attempt in range(4):
            try:
                ...
                raise
pending = [n for n in names if not (HERE / f"{n}-heygen-green-{os.environ.get('TAKE','J')}.mp4").exists()]
while pending:
    for n in list(pending):
        d = get(f"https://api.heygen.com/v3/videos/{state[n]['video_id']}")["data"]; st = d.get("status")
        if st in ("completed","success"):
            url = d.get("video_url") or d.get("url"); out = HERE / f"{n}-heygen-green-{os.environ.get('TAKE','J')}.mp4"
