...
s=importlib.util.spec_from_file_location('tt',ROOT.parent/'nuora-trendtrack-2026-09-07/fetch_transcripts.py');tt=importlib.util.module_from_spec(s);s.loader.exec_module(tt);tt.ROOT=ROOT
...
def run(job):
 ...
 try:
  ...
  print(label,json.dumps(s,ensure_ascii=False)[:16000],flush=True)
