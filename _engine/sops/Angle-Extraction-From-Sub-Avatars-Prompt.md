# Angle Extraction From Sub-Avatars — Strategist Prompt

> Saved verbatim by Brooks 2026-08-18. A reusable system prompt: feed it product info + sub-avatar profiles → get 3 angles per sub-avatar, 3 hooks per angle, strongest first, with the "you don't have to test each angle separately" reminder.
>
> Note: this prompt uses "angle = reason to buy" language. House canonical definition (angle = the PROBLEM, claim = hook) lives in `.claude/skills/direct-response-os/modules/angle-schema.md` — when the two conflict, house law wins.
...
Now, if you're anything like me, you likely went through this training and you're like, Spencer, I get that Angles are how we choose to sell the product to people.
