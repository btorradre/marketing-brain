...
This document describes how to compute and report TRUE contribution margin (not just revenue) for a DTC brand, and how to reason about the resulting numbers. Use it whenever asked about contribution margin, CM1/CM2/CM3, true profit, net margin, COGS accuracy, MER, CAC, blended ROAS, "what did we actually make," "are we profitable," or when refreshing/reporting a margin/finance dashboard.

The original implementation scraped every Shopify order for real per-order COGS, pulled ad spend/payment fees/channel attribution from Triple Whale, pulled chargebacks from Shopify Payments disputes, pulled operating expenses from QuickBooks, and amortized monthly fixed costs to a daily rate — then published the result to a password-protected web dashboard, refreshed nightly. On a different platform, replicate the same logic against whatever equivalent data sources are available (store order/order-line data, ad platform spend data, payment processor dispute data, accounting-system fixed costs).
...
7. Publish/report the result with the ladder, not just top-line revenue, and flag any number that is estimated rather than measured.
...
- **Shipping — often not measured at all** unless you specifically wire up a shipping-cost source (see known gaps below).
