---
name: swipe-intake
description: Processes a queue of raw competitor/reference ad data (exported from an ad-spy tool as JSON records) into a well-organized reference library — classifies each ad by type, transcribes video content, tags each ad with a full metadata taxonomy, writes a formatted record, files it into the correct reference collection, and updates the index. Use whenever there's a new batch of exported ad data to process into a research/swipe library — "process the queue," "classify these ads," "ingest these swipes," or any variation of turning raw ad-spy exports into filed, tagged reference material.
---

# Swipe Intake — Ad Classification & Routing System

This skill turns a queue of raw competitor/reference ad data (exported from an ad-spy tool as JSON records — one per ad, containing body copy, title, landing page URL, display format, media/video URLs, performance score, days active, start date, and brand tags) into a well-organized reference library: each ad gets classified by type, video content gets transcribed, each ad gets tagged with a full metadata taxonomy, a formatted record is written, it's filed into the correct reference collection, and an index is updated.

## Golden nugget doctrine (apply before any downstream analysis)

Before drafting any classification notes or downstream copy work built from this material, apply the golden nugget doctrine: identify the single most emotionally loaded deep frame the ad is built on — the real motive that makes the buyer act, never just the surface topic.

- **Topic ≠ motive.** "Memory loss" is a topic; "I thought I was getting dementia just like my mother did, until I discovered this" is the frame. Surface angles buy mild hope or curiosity; deep frames trigger identification so strong the reader feels caught.
...
- When analyzing a reference ad instead of writing new copy, state the nugget it's built on and whether the ad actually leads with it.
