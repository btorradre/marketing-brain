...
description: Takes any reference AI-generated UGC-style video ad (podcast format, talking head, multi-person, product demo, etc.), extracts keyframes at every scene shift, transforms each via image-to-image, animates the results, lip-syncs to a new voiceover, polishes camera motion, and stitches the final video — replicating the reference's structure and visual style for a different brand. Not limited to any single ad format. Use when the goal is to replicate an existing AI-generated video ad's structure for a new brand/product.
---

# AI UGC Video Replicator

Replicates any AI-generated UGC-style video ad (podcast, talking head, multi-person, product demo, etc.) for a new brand: extracts keyframes at every scene shift, transforms each via image-to-image, animates them, lip-syncs to a new voiceover, polishes camera motion, and stitches the final video.

## Golden Nugget Doctrine (apply first)

Before any hook, angle, script, or adaptation decision is made, name the golden nugget: the single most emotionally loaded deep frame in the reference or brand research — the real motive that makes buyers act, never the surface theme. Topic ≠ motive — dig one layer deeper than the surface theme (e.g. weight loss → the stolen victory; bloat → exiled from my own dinner table). State the golden nugget in one explicit sentence before adapting. If it doesn't surface from what's given, mine the brand's reviews/VoC until it does — never default to a surface angle.
...
[Stage 1] Dual-mode keyframe extraction
    |       TWO passes:
    |       Pass A: scene-change detection (threshold ~0.25) for hard cuts
...
[Stage 2] Asset classification & deduplication
    ...
    |         - STANDALONE_OVERLAY: generate as an isolated image (NO talking head)
    |         - FULLSCREEN: generate as a full-frame image
    |         - PRODUCT_SHOT: generate with real product reference photos
    |         - TALKING_HEAD: skip (avatar handled by a separate process)
    v
...
- **Pass A — scene detection.** Extract a frame every time the scene-change score crosses a moderately sensitive threshold (around 0.25 — err toward more sensitive than a typical default, to catch more transitions). This gets hard cuts: full-screen B-roll, scene changes, avatar switches.
...
## Faster path for straightforward talking-head UGC

For the bread-and-butter case — a single-avatar talking-head UGC ad with no complex overlay graphics — a one-shot "marketing studio"-style video generation tool with native avatar consistency and native lip-sync can produce an equivalent or better result in a single pass, without needing the full 10-stage pipeline above (no separate scene-detection, auditor, or motion-control passes).

Keep running the full pipeline in this document when:
- The reference is a multi-person podcast or has complex overlay graphics that need the Stage 0 analysis + auditor pipeline.
- The user wants high-fidelity lip-sync on a custom voiceover that a one-shot tool's native TTS won't match.
- The reference style doesn't fit any available one-shot preset.
