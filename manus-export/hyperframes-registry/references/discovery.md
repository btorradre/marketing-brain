...
For an always-current list run `npx hyperframes catalog --type block`. The tables below group the 97 blocks by category. **Block name ≠ shader name**: shader-transition blocks (e.g. `domain-warp-dissolve`) wrap a shader runtime whose internal name omits the `-dissolve`/`-warp` suffix — see the showcase HTML installed alongside the block for the canonical name.

#### Shader transitions (14)

Single-shader blocks; each installs one shader runtime + a showcase composition. Use ≤2 per video.
...
| `flash-through-white`    | White flash crossfade — rarely a neutral default; choose it deliberately for the tone of the cut rather than as a fallback |
...
WebGPU + html-in-canvas frosted-glass surfaces. **Require a WebGPU-enabled Chromium-based browser** (Chrome canary or Brave) — the headless render shell needs to point at that browser binary, and the engine auto-passes `--enable-unsafe-webgpu`. These are advanced/experimental blocks; treat WebGPU availability as a precondition before relying on them.
...
HTML-in-canvas + WebGL composition blocks, driven by Three.js and canvas-in-3D compositing techniques (see `hyperframes-keyframes` for the underlying seek-safe canvas/WebGL keyframing rules if hand-authoring similar motion).
