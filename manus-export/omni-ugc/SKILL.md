---
name: omni-ugc
description: Turns a single reference image into a finished AI UGC-style "reaction ad" — a short vertical video of an AI-generated actor reacting to or talking about a product — using either Google's Omni video model (10s fixed segments) or Seedance 2.0 (5-15s flexible segments). Covers actor-keyframe generation, beat-paced script writing against a locked words-per-second table, engine-appropriate identity/voice anchoring so nothing drifts across segments, and stitching into a final vertical video. Use when the user hands over a reference image (e.g. a Pinterest-sourced photo establishing a look/vibe) and wants a multi-beat AI UGC reaction video, or says "turn this into an AI UGC ad", "run this on Omni/Seedance", or similar.
---

# Reference Image to AI UGC Reaction Ad

This skill turns a single reference image into a finished AI UGC-style "reaction ad" — a short vertical video of an AI-generated actor reacting to or talking about a product — using one of two video generation engines: Google's Omni model (10s fixed segments) or Seedance 2.0 (5-15s flexible segments). Use whenever you have a reference image and need a multi-beat UGC reaction video with a consistent actor and voice across segments. Reference sourcing (finding the Pinterest image or vibe board) is a manual step that happens before this pipeline starts — this skill only covers generation and stitching.
...
- Persist in-flight task IDs so that re-running the same job resumes polling instead of creating (and paying for) a duplicate task.
...
Product segments swap the "NO PRODUCT IN HAND" line for: `PRODUCT IN HAND, LABEL FACING CAMERA, LABEL TEXT UNCHANGED.` and attach the product reference image as an extra input. You only author the movement line, voice line, and dialogue per beat — everything else stays fixed, on both engines. Do not write dense, over-engineered prompts, and do not add camera vocabulary, timestamps, or extra instructions to the template. Segment duration is a job-spec field, never something written into the prompt text.
