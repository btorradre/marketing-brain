...
Working reference for DR marketers running the general/interpretive image-to-video model class (e.g. Kling 3.0). Focus: animating static product/lifestyle stills into UGC-feeling motion that converts.
...
The formula converges on "Camera Movement + Scene Setup + Subject Action + Vibe + Time/Audio"; for i2v drop "Scene Setup" because the frame already provides it.

## 2. Motion vocabulary cheatsheet

**Camera verbs this model reliably honors:** `dolly in slow`, `dolly push`, `pull back`, `whip-pan`, `tilt up/down`, `orbit left`, `crane up`, `crash zoom`, `snap focus`, `rack focus`, `shoulder-cam drift`, `static tripod`, `handheld follow`. Compound with `while` or `as`.
...
**Words this model ignores or muddles:** `moves`, `goes`, `does something`, `interacts`, `looks nice`, `cinematic`. Generic verbs produce mush.
...
**≤2 sentences is the sweet spot for i2v.** Long prompts fight the start frame. This class of model "can work great without the prompt — add small details when you need exact camera or action control." Past 60 words, you're over-cooking it.

## 4. Standard vs. pro mode
...
Workflow: explore in standard mode, finalize hero shots in pro. Pro mode reduces re-rolls.

## 5. Sound on vs. off

Native audio generates **dialogue (lip-synced if a face is in frame), SFX synced to motion events, and an ambient bed (room tone, wind, traffic) — all in one pass**.
...
Turn sound off when dubbing an external voiceover over the top. Turn it on for talking-head UGC where native dialogue beats lipsync-in-post.

## 6. Duration tradeoffs

This model class typically supports 3–15s. Practical sweet spots:
...
Rule: 2+ subjects or rapid action → hard-cap 5–6s and chain clips in post.

## 7. Aspect ratios

9:16, 16:9, 1:1 supported. No documented quality delta — but the start frame's AR drives the clip, so feed the model the AR you want out.

## 8. Start-frame + end-frame mode

Pass both a start image and an end image for controlled interpolation. Rules:
...
- **Multi-clip sequences:** export the last frame of clip N as the start frame of clip N+1. Cleanest way to fake 15s+ shots.

Breaks when: faces rotate >45°, products change orientation, or one frame has motion blur and the other doesn't.

## 9. Camera control parameters

Beyond text prompts, this model class often exposes numerical sliders in pro/i2v mode: horizontal, vertical, zoom, pan, tilt, roll. **Keep values low (1–3) for cinematic smoothness;** higher values distort. Presets (pan, orbit, zoom, handheld, stationary) are often available as named parameters.

## 10. Identity consistency

What helps:
- Toggle a "bind subject" / element-reference feature if the platform offers one.
...
| Failure | Fix |
|---|---|
| Face morphs mid-clip | 5s only; bind-subject feature; remove face descriptors; pro mode |
| Plastic-skin slow-mo | Add `realistic skin texture, visible pores, natural micro-expression`; avoid `cinematic, dreamy, smooth` |
| AI face wobble (talking head) | Use native audio (sound on) instead of dubbing; keep mouth motion implied not described |
...
| Glossy AI-render feel | Add the 4 imperfection cues (below) |

## 12. The "4 organic imperfection cues" pattern

Handheld jitter + rolling shutter + eye-focus drift + background motion maps directly to what every credible guide for this model class recommends under different labels. This isn't superstition — it's the documented difference between AI render and phone capture. Use it on every UGC clip.
...
**Car-yapper UGC (sound on, standard mode, 5s, 9:16):**
```
...
**Kitchen counter product reveal (pro mode, 5s, 9:16):**
```
...
**Gummy bottle being opened (pro mode, 5s, sound on, 1:1):**
```
...
**Before/after pan (standard mode, 5s, 16:9, end-frame mode):**
```
Slow horizontal pan left, steady. Soft daylight, no other motion.
```

**Fashion outfit walking shot (pro mode, 6s, 9:16):**
```
