---
name: dr-hook-lab
description: Writes five genuinely different hook variations for one angle, each aimed at one specific person at one awareness level, in the customer's own recorded language. Use when someone says "write hooks," "give me hook variations," "5 hooks for this angle," "new opening lines," "the hook isn't landing," "rewrite the first 3 seconds," or picks an angle to brief.
---
...
7. **Close with two lines**: the recommended hook, and the next step — for video, a creator brief that stages the hook into shots and demonstrates the claim on screen; for long-form, a full copywriting pass that builds the rest of the piece out from this opening; for statics, an image-ad prompt pass that turns the hook into a headline treatment.
...
sophistication: 3                   # 1-5: how many times has this market already heard this exact claim
...
hooks:                              # the ad variations this angle spawns
  ...
  - text: "During menopause your skin loses minerals"
    ...
    status: fresh
    verdict: ""
swap_test: pass                     # pass | fail — a record that fails cannot be briefed as-is
...
**Mechanism** is a supporting term, not a layer of its own: it's why the problem exists, the ingredient inside a hook that makes it believable (progesterone decline is a mechanism). **Golden nugget** is likewise not a competing layer — it's the depth requirement on the angle, and the hook is where it gets voiced.

Naming convention for tracking hooks once shipped: `<AVATAR>_<ANGLE-ID>_<HOOK-N>_<FORMAT>`, e.g. `MENOSKIN_MOT-A-014_H2_UGC`.

## House laws this skill enforces most directly

**Law 9 — Voice tells that kill a draft** governs every line written here (see the gates table above): no em dashes, no "That's not X. It's Y." or variants, no parallel repetition stacks, no rhetorical section openers, no "Imagine if" openers, no tidy tricolons or symmetrical parallelism, never personify objects.

**Law 2 — Nothing enters the output unsourced.** Every hook's source language is a verbatim quote traceable to real customer research. Never invent a study, sample size, percentage, doctor, press mention, review count, or customer quote. Format: `"exact quote" — (source, date)`. A hook with no source quote says so explicitly.
...
**Law 5 — The swap test.** Drop a competitor's product into the hook. If it still reads perfectly, it sells the category, not the brand. Fix: lead with a verdict, then stack the brand's own physical facts.
...
**Law 7 — Every claim gets a demo beat.** A hook that implies a claim needs a demonstration beat downstream, not just a description.

**Law 8 — The creator is never the brand.** A UGC creator delivering the hook says "they're," "their," "this brand" — never "our," never "we."

**Law 11 — Self-audit before presenting.** Run every line against Laws 2, 4, 5, 6, 7, 9 before it's shown to anyone.
