...
The `hash()` function is deterministic — same frame renders identically every time. This matters because the underlying renderer captures frames deterministically; `Math.random()` would render differently every time the composition is re-captured.
...
Words appear one-by-one, synced to word-level transcript timestamps. The core technique for narration-driven videos.
...
<script>
  // Video playback is controlled by the rendering framework — don't call play() manually
  tl.from(".video-frame", { scale: 0.9, opacity: 0, duration: 0.3, ease: "power2.out" }, 0);
</script>
```

The rendering runtime handles video seeking and playback deterministically.

---

## 7. Character-by-Character Typing

Terminal typing effect using timeline callbacks to update text content character by character.
...
```html
<style>
  /* Load the captured local variable font — do NOT use a Google Fonts @import.
...
**How it works:** Pre-extract audio frequency bands into a JSON file, then sample per-frame via a timeline callback:
...
Per-frame sampling is required — a single tween will not react. Extract the audio-band data with a small offline script: run an FFT over fixed-size windows of the narration/music waveform at your target fps, average magnitude into N bands per window, and write `{fps, totalFrames, frames: [{bands: [...]}]}` to JSON.
...
**Never do:** equalizer bars, spectrum analyzers, waveform displays, strobing, rainbow color cycling. The audio provides _timing and intensity_; the visual vocabulary still comes from the brand.
