---
name: aiugc-longform
description: Produce 60-180 second single-shot, continuous-take, creator-talking-head VSL (video sales letter) ads — one creator, one continuous setting, one continuous voiceover, no B-roll, no scene cuts, with a hook overlay burned into the first 8 seconds. Use for "long-form UGC", "VSL-style talking head", a "60-180s continuous yapper", or replicating ads in the style of a single continuous-shot creator rant (the reference pattern is a joint-pain/menopause supplement ad style, "HurAgain"). Distinct from a two-cut format that hard-cuts from talking head to product-action b-roll — this format never cuts away from the creator at all.
---

# AIUGC Longform — Single-Shot Continuous Talking-Head VSL Factory

Produces 60-180 second single-shot, continuous-take, creator-talking-head VSL (video sales letter) ads: one creator, one continuous setting, one continuous voiceover, no B-roll, no scene cuts, with a hook overlay burned into the first 8 seconds.

**When NOT to use this:** for a two-cut format (talking head, then a hard cut to product-action b-roll), use a two-cut UGC pipeline instead (see `aiugc-infinite`). For short 15-30s UGC, a simpler short-form pipeline is more appropriate. For replicating one specific existing reference video exactly, use a video-replication process instead of generating original scripts.
...
## THE ONE-TAKE AUDIO LAW (read before choosing an approach — the load-bearing rule of this whole skill)
...
[3] For each VSL script (loop):
       ...
       c. For each chunk, generate a lip-synced video segment seeded from the SAME
             ...
             frame — see below), using that chunk's audio as the lip-sync reference
             and a motion-only prompt (no dialogue text in the prompt).
...
Before generating a hook or a VSL script, name the **golden nugget** — the single most emotionally loaded deep frame in the research, the real motive that makes buyers act, never the surface theme.
...
- The golden nugget leads — it IS the hook, at the very top. Never buried in the body.
- State the golden nugget in one explicit sentence before drafting. If the brand's research hasn't surfaced one, mine reviews/VoC first rather than defaulting to a surface angle.
...
The script must be pure continuous spoken text — no scene markers, no sound-effect notation — and must follow the seven-beat arc described in the reference pattern (see `references/huragain-pattern.md` for the full distilled breakdown, word-count guardrails per beat, and worked examples).
...
**Tonal consistency** (anti-monotone, anti-drift): the known failure pattern for AI voice delivery is an energetic opening that gradually decays into monotone by the back half, and this compounds across a long chain of segments. Counter it with:
1. One named base vocal register, restated verbatim in every chunk's prompt, with only small shading deviations between chunks ("warmth lifts slightly") — never a full gear change.
2. Explicit instruction in every prompt that energy holds through the final word of the clip, with no trailing off or fading to flat.
3. Every segment from the second onward should anchor its voice reference to segment 1's audio specifically, never to the immediately preceding segment's audio (to avoid compounding drift).
4. Listen to every clip's last sentence specifically for energy fade, and regenerate any faded segment before letting a later segment build on it.
5. Spot-check the first, a middle, and the final segment against each other for register drift.
...
## Reference pattern — "HurAgain" single continuous-shot style

Full distilled analysis (structural skeleton with word counts per beat, voice/cadence rules, visual pattern, hook overlay exact specs, and why the format converts) lives in `references/huragain-pattern.md`. Summary:

- Single continuous selfie phone shot, low-angle, eye-level to low-chest framing.
- One creator, one setting, no cuts, no B-roll, no zoom, no pan.
- Two settings that work well: a kitchen with wooden cabinets (waist-up) or a parked car in the driver's seat (chest-up).
- Practical/ambient lighting only — no softboxes, no visible ring lights.
- Hook overlay: red rounded badge, bold white sans-serif, 2-3 lines, pinned 0-8s.
- Voice cadence: conversational "rant" energy, granular sensory specificity, authority subversion ("doctors said X, but..."), mechanism explained in plain English, proof carried by metaphor more than data, soft-sell CTA.
...
- **Trim leading silence from every audio chunk** before generating its video segment.
...
- **Audio seams / "the segments don't flow into each other"** → this is the single most common long-form failure and the entire reason the One-Take Audio Law exists. It's caused by generating audio per-clip, so every segment is a separate performance. Symptom: each join sounds like the speaker restarting, even with clean words and no measurable gap. This is NOT fixable by chunking, trimming, crossfading, or re-encoding. The only fix: render one continuous voiceover and replace every segment's own audio with it at the final stitch. If the chosen video-generation engine cannot lip-sync to a supplied audio track, it is the wrong engine for long-form work.
...
Per-VSL cost is high relative to a short two-cut ad — a 120s VSL typically needs 10-15 video segments plus a full-script TTS pass. Keep batches small by default (N=3) unless the user asks for more. Reduce cost by using a faster/cheaper generation mode, shortening target duration (fewer segments), or rendering at a lower resolution for first-pass iteration and only finalizing winners at production resolution. Always run a cost preflight against account balance before starting a batch, and confirm with the user if the estimated total is high relative to balance.

## Output structure

```
output/run_<brand>_<timestamp>/
├── scripts.json
├── manifest.json
├── vsl_001/
│   ├── script.json
│   ├── vo_full.wav              # the one continuous voiceover — THE audio track
│   ├── vo_timestamps.json
│   ├── chunks/
│   │   ├── chunk_001.wav
│   │   ├── chunk_002.wav
...
├── vsl_001.mp4                   # copy of vsl_001/final.mp4 for convenience
├── vsl_002/...
└── vsl_003/...
```
...
After a successful run, persist: the working avatar keyframe for this brand (so future batches don't have to regenerate it), a winning voice if the user confirmed one, and the ids of winning VSLs for future remixes.
