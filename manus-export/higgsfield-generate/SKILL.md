...
description: Generate images and videos through the Higgsfield AI platform using its official CLI. Covers the full model catalog (GPT Image 2, Nano Banana 2/Pro, Soul V2/Cinema/Cast/Location, Seedance 2.0, Veo 3.1, Kling 3.0, Flux 2, Z Image, Hailuo, and more) plus Marketing Studio for branded ads with avatars, products, hooks, and settings. Use whenever the user asks to generate an image, make a picture, make a video, animate a photo, do image-to-video, edit/stylize/remix an image, produce a clip, create an ad, make a UGC video, product demo, unboxing video, TV spot, brand video, or presenter video, or import a product from a URL for an ad. Supports text-to-image, image-to-image, image-to-video, reference-based generation, and Marketing Studio. Not for training a persistent identity/Soul Character, running a dedicated product-photoshoot pipeline, generating marketplace listing cards, or text/chat/TTS tasks.
---

# Higgsfield Generate

Submit generation jobs to any Higgsfield model via the `higgsfield` CLI. Covers generic image/video generation and Marketing Studio (branded ads with avatars, products, hooks, settings).
...
1. If `higgsfield` is not on `$PATH`, install it:
   ...
   ```
2. If `higgsfield account status` fails with `Session expired` / `Not authenticated`, have the user run `higgsfield auth login` (interactive, opens a browser) and wait for them to confirm before continuing.

Skip both checks if `higgsfield account status` already prints account info.

## Operating rules

1. Be concise. No raw IDs, no JSON dumps in the reply. Report the result URL when ready.
2. No internal jargon in front of the user — don't narrate "calling higgsfield cost" or "polling job."
3. Detect the user's language from their message and reply in it. Technical args (`--aspect_ratio 16:9`) stay in English.
4. Don't batch-ask for parameters. Pick a sane default model and ask one thing at a time, only if genuinely missing.
5. Don't pre-estimate cost unless asked. Just submit.
...
1. **Pick a model.** Practical defaults from production use:
   ...
   **Image:**
   - Brand product visual (Pinterest pin, lifestyle shot, hero banner, ad pack, virtual try-on) — this general-purpose skill is the wrong tool; that job wants a dedicated product-photoshoot pipeline (a specialized GPT Image 2 workflow with ~10 mode-specific prompt templates tuned for exactly those shot types). Route there instead if that pipeline is available to you; otherwise approximate it here with GPT Image 2 and a carefully written prompt.
   ...
   - Soul Character (a trained, reusable identity reference — see "Identity-locked characters" below) → Soul 2.0 for stills, Soul Cinema for cinematic
   ...
   **Video:**
   ...
   For the actual `--model` ID to pass to `higgsfield generate create`, run `higgsfield model list --json | jq` to map display names to IDs. See `references/model-catalog.md` for the full table and the full picking-flow decision tree.

2. **Pass media inputs straight to flags.** Media flags accept a local file path **or** a UUID. The CLI auto-uploads paths and auto-detects job vs. upload for UUIDs. No need to pre-upload. Each model declares accepted roles (`image`, `start_image`, `end_image`, `video`, `audio`) — see `references/media-inputs.md`.
3. **Validate quickly.** If unsure of params, run `higgsfield model get <model_id> --json` once and pass only what's needed. Use schema defaults otherwise. The server returns `adjustments` for non-fatal coercions (e.g. `aspect_ratio=99:99` → closest match) and a structured error for invalid declared-param values.
4. **Submit and wait in one shot.** `higgsfield generate create <model_id> --prompt "..." [media flags] [param flags] --wait`. Blocks until terminal status and prints the result URL on stdout. Tunables: `--wait-timeout 20m` (default 10m), `--wait-interval 5s` (default 3s).
5. **Deliver.** Send the URL plus a one-line summary (model, duration if video).

To inspect or rerun later, `higgsfield generate list --json` and `higgsfield generate get <id> --json` work for retrospection. `higgsfield generate wait <id>` is still available if you need to rejoin a job started without `--wait`.
...
Flags pass through to the model schema. Use `higgsfield model get <model_id>` to discover them.
...
For prompt-writing guidance (subject/setting/style structure, image-to-image phrasing, image-to-video motion phrasing, negative-phrasing workarounds), see `references/prompt-engineering.md`.

## Identity-locked characters (Soul Character)

When the user wants the same face/identity to recur consistently across multiple generations, that requires training a reusable Soul Character first: upload 5–20 reference photos, then train it (roughly a 10-minute one-time process). Once trained, its `soul_id` can be passed into Soul 2.0 / Soul Cinema generations (`--soul-id <id>`) for identity-locked output. This skill covers using a Soul Character once one exists; it does not cover the training flow itself.
...
One question per phase. Don't ask product+avatar+mode all at once.
...
1. **Get product.**
   ...
   Capture the product id. When using `--hook_id`, strongly prefer passing `--product_ids`; hooks are designed to pivot into a product and work poorly without product context.
...
3. **Optionally pick setup items.**
   ...
   Pass selected IDs as `--hook_id <hook_id>` and `--setting_id <setting_id>` for `marketing_studio_video` only. Do not copy the hook's prompt into `--prompt` unless the user explicitly wants to reinforce the same wording. See `references/marketing-setup-items.md`.
...
- `Unknown params: foo` → schema doesn't accept that flag; check `higgsfield model get <model_id>`. If this happens for `hook_id` or `setting_id`, the selected model/job type does not support Marketing Studio setup items.
- `Session expired` → `higgsfield auth login`.

See `references/troubleshooting.md` for the full list of errors and fixes.

## Reference docs

Load on demand:

- `references/model-catalog.md` — full model table + picking-flow decision tree for every image and video model
- `references/prompt-engineering.md` — writing prompts that work
- `references/media-inputs.md` — image/video reference flows, role tables per model
