...
description: Turns a finished ad script plus a set of already-generated B-roll/scene assets (produced by a prior scene-replication pass) into a complete, production-ready brief for a human video editor — matching each script line to the best-fitting generated scene, then assembling a timeline, visual direction, motion notes, file references, and an asset manifest. Use after a script and a set of generated/animated scene assets both exist and need to become a ready-to-execute editor brief.
---

# Video Editor Brief Generator

This document describes a process for turning a finished ad script plus a set of already-generated B-roll/scene assets (produced by a prior "video scene replication" pass) into a complete, production-ready brief for a human video editor. It matches each line of the script to the best-fitting generated scene, then assembles a full brief with a timeline, visual direction, motion notes, file references, and an asset manifest. Use this after you already have a script and a set of generated/animated scene assets, and need to hand off a ready-to-execute brief to an editor.
...
Building this as an automated pipeline typically needs:
- A multimodal/text-reasoning API call to do the script-to-scene matching in Stage 3 (feed it the full script plus every scene's structured JSON description in one prompt, and ask it to return a JSON array of `{segment_number, segment_text, matched_scene, confidence, editor_notes}` objects, plus a final entry listing any unmatched scene numbers).
- A fallback matching strategy (simple sequential 1:1 assignment) for when the matching call fails, so the pipeline never dead-ends.
- Wherever the brief and its paired images are ultimately published (a shared document, a workspace page, a project tracker), keep the credentials/tokens for that destination out of any file that leaves your own environment.
