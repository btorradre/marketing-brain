...
## Chart-type selection database (verbatim from source)

Source: `charts.csv` — 25 data-type-to-chart-type mappings (Trend Over Time, Compare Categories, Part-to-Whole, Correlation/Distribution, and more). Each entry gives: matching keywords, the best chart type, secondary chart options, when to use / when NOT to use, a data-volume threshold with rendering guidance (SVG vs Canvas vs aggregation), color guidance, a WCAG accessibility grade (A-AAA) with accessibility notes, a mandatory a11y fallback, library recommendations, and the expected interactive level.
