# Style Catalog (full database)

This is the full verbatim style database backing the "Style Selection" rule category in `references/style.md`. It holds 84 named visual styles (Minimalism & Swiss Style, Glassmorphism, Claymorphism, Brutalism, Neumorphism, Bento Grid, Skeuomorphism, Dark Mode variants, and many more), each with: primary/secondary colors, effects & animation notes, best-for and avoid-for product types, light/dark mode support, performance and accessibility ratings, mobile-friendliness, conversion focus, framework compatibility scores, era/origin, complexity, ready-to-use AI prompt keywords, CSS/technical keywords, an implementation checklist, and design-system CSS variables.

Use it by matching product type + audience + style keywords (from the SKILL.md workflow) against the "Keywords" and "Best For" fields below, then pull the "Do Not Use For" field to check for anti-pattern mismatches before committing to a style.
...
- **CSS/Technical Keywords:** borderWidth: 4 (primary), 2 (secondary), borderRadius: 0 or 999 (badges only), backgroundColor: '#FFFDF5', shadow implemented as offset View, transform: [{translateX:4},{translateY:4}] on PressIn, fontFamily: 'SpaceGrotesk-Bold', fontWeight: '700/900', transform: [{ rotate: '-1deg' }] on cards, padding: 20,☐ 4px borders on major elements
- **Implementation Checklist:** ☐ Hard offset shadow implemented via extra View
- **Design System Variables:** ☐ Mechanical press hides shadow
...
- **Design System Variables:** --bg: #1C1714, --bg-alt: #251E19, --fg: #E8DFD4, --muted: #3D332B, --muted-fg: #9C8B7A, --border: #4A3F35, --accent-brass: #C9A962, --accent-crimson: #8B2635, --radius: 4px, --arch-radius: 100px, --shadow-card: 0 4px 6px rgba(0,0,0,0.4), --font-heading: Cormorant Garamond, --font-body: Crimson Pro, --font-label: Cinzel
