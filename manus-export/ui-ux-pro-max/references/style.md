# Style Selection (Priority 4 — HIGH)
...
- The full 84-entry style catalog (Minimalism, Glassmorphism, Claymorphism, Brutalism, Neumorphism, Bento Grid, Skeuomorphism, etc. — each with primary/secondary colors, effects, best-for/avoid-for guidance, framework compatibility, and AI prompt keywords) lives in `references/style-catalog.md`.
- Product-type → style/color/pattern recommendations and the decision-rule reasoning engine live in `references/product-design-system.md`.
- Font pairings live in `references/typography-pairings.md`; color palettes in `references/color-palettes.md`.

## Icon library (verbatim from source)

Source: `icons.csv` — 105 curated icons across Navigation, Action, Status, Commerce, and other categories, each with the exact import statement (Phosphor / Heroicons / Lucide) and recommended usage.
