...
Drop-in animation patterns for HTML/CSS motion-graphics compositions. Each effect is self-contained with HTML, CSS, and code.

All effects follow deterministic-composition rules — no randomness, timelines built synchronously so a rendering/capture step can read them predictably.
...
Pre-extract audio data, drive canvas/DOM rendering from a GSAP timeline.
...
**Do NOT use async `fetch()` to load audio data.** Deterministic compositions require synchronous timeline construction — a capture/render step often reads the timeline object synchronously right after page load. Building timelines inside `.then()` callbacks means the timeline isn't ready in time.
...
**WebGL / Three.js** — patch `THREE.Clock` for deterministic time if the renderer requires frame-exact playback. Update uniforms from audio data each frame.
