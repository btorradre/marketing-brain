...
Practical lever-set for direct-response creative. Model: OpenAI `gpt-image-2` (Higgsfield exposes it as `gpt_image_2`). All guidance below is model-specific; ignore Midjourney/SD 2023-era tags ("8k, masterpiece, trending on artstation") — they do nothing here.

---

## 1. Prompt structure (canonical order)

OpenAI's official cookbook recommends a **5-block structure**, not subject-first:
...
| Use GPT Image 2 for | Switch to another model when |
|---|---|
| On-image typography / headlines / packaging copy (95%+ accuracy, Latin + CJK) | Need a tight character-identity lock across many shots → **Higgsfield Soul ID** |
| Editorial / studio / "designed" creative | Need ultra-fast bulk variations → **Flux 2** or **Z Image** |
| Surgical edits with up to 16 references | Need photoreal i2i transformation of a real product photo with brand-faithful colour → **Nano Banana 2/Pro** |
| Multilingual text | Need cinematic stylized look → **Seedream / Soul Cinema** |

Default to Nano Banana 2 for hero pack-shot i2i. Use GPT Image 2 when you need (a) on-image text, (b) compositional reasoning, or (c) a layout/poster.
...
6. Use `quality: high` for dense or small text.
...
GPT Image 2's edit endpoint takes up to **16 reference images** and processes every reference at high fidelity automatically. The `input_fidelity` parameter is locked-on — there is no "strength" knob. Control fidelity through **language, not parameters**.
...
- **Downscale references** before upload — 4K refs cost the same tokens as 1024px and don't help quality.

---

## 5. Aspect ratios & resolutions

API constraints: max edge < 3840px, both edges multiples of 16, long:short ≤ 3:1, total pixels 655,360–8,294,400.
...
Above ~2560×1440 is **experimental**. For DR work, render at `quality: low` or `medium` then upscale with a dedicated upscaler.

---

## 6. Multi-image composition / inpainting

- Edit endpoint accepts up to 16 input images.
...
- For variations: edit small wording or use the `n` parameter.
...
| Failure | Fix |
|---|---|
| Hand/finger artifacts | Specify exact hand action: `right hand cradling bottle from base, four fingers visible wrapping around, thumb on label edge`. Crop tighter. Use `quality: high`. |
...
| Body part distortion | Tighten framing. State pose explicitly. Avoid full-body 9:16 with small product — split into two shots. |
| Repeated/duplicate product | `single bottle only, no other bottles in frame, no reflections of the bottle`. |
| Watermark hallucination | `no watermark, no stock photo overlay, no Getty bug, no signature`. |
| Brand/IP refusal | Use generic descriptors: `a green-and-white auto-injector pen` not `Ozempic`. |
...
| AI-glossy product render | Pass real product reference and write `photographic, not CGI, not render, not 3D — taken on a real camera`. |
...
| Avoid | Tag spam, mood words, brand names of competitors, full-body distance shots |

**Sources:** OpenAI Cookbook (image-gen-models-prompting-guide, generate_images_with_high_input_fidelity), OpenAI Image Generation API docs, Higgsfield gpt-2 page, fal.ai prompting-gpt-image-2, Atlabs ultimate GPT Image 2 prompting guide 2026, WaveSpeed GPT Image 2 API guide.
