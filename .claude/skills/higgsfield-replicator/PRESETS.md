...
The slug below is what you pass to `higgsfield generate create marketing_studio_video --mode <slug>`. (The `marketing-studio` widget UI surfaces names like "Tutorial" and "Hyper Motion" — the CLI mode enum uses `ugc_how_to` and `product_showcase` for those. Always use the CLI slug.)
...
When the user just says "make a Higgsfield ad for Motilli" without a reference, default to:
...
Override these whenever the reference clearly fits a different preset.

## Reference-Type → Preset Examples

| Reference description | Preset |
|---|---|
| GetHookd UGC car-yapper ad | `UGC` |
| Animated "inside the gut" 3D ad (Motilli/Serene Herbs style) | `Wild Card` (custom prompt) — Higgsfield doesn't have a native animated-3D preset |
| Talking-head doctor explaining a mechanism | `UGC` (with authority avatar) or `Product Review` |
| 30s broadcast spot with dramatic music | `TV Spot` |
| Slow-mo gummy splash into water with ingredients flying around | `Hyper Motion` |
| "I tried Motilli for 14 days, here's what happened" before/after | `Product Review` |
| Outfit-of-the-day featuring a Velantra bag | `UGC Virtual Try On` |
| Editorial Vogue-style shoot featuring Velantra Meridian | `Pro Virtual Try On` |
| Recipe video using a kitchen product | `Tutorial` |
| Box reveal + unwrapping + product discovery | `Unboxing` |
| Surreal "person on volcano rim casually reviewing the product" | `Wild Card` (or `UGC` + Volcano Rim setting) |
...
For animated/3D references that don't match any Higgsfield preset, fall back to the existing `animated-video-replicator` (Veo 3.1) — Higgsfield has no native 3D animation preset.
