...
description: >
  ...
  identical and sacred — Velantra's warm ivory palette, New York 400 + Abel, the
...
typography:
  # — reading ramp (Abel) —
  body:        { fontFamily: "Abel", cqw: 1.0,  weight: 400, lineHeight: 1.6, color: "text-secondary" }
  body-sm:     { fontFamily: "Abel", cqw: 0.85, weight: 400, lineHeight: 1.6 }
  subtitle:    { fontFamily: "Abel", cqw: 1.3,  weight: 400, lineHeight: 1.5 }
  label:       { fontFamily: "Abel", px: 14, weight: 600, tracking: "3px", upper: true, color: "accent" }
  attribution: { fontFamily: "Abel", px: 15, weight: 400, tracking: "2px", upper: true, color: "accent" }
  micro:       { fontFamily: "Abel", px: 12, weight: 400, tracking: "2px", upper: true, color: "accent" }
  # — display / hero ramp (New York 400, sentence case) —
  h3:          { fontFamily: "New York", cqw: 1.8, weight: 400, lineHeight: 1.1 }
  timeline-headline:{ fontFamily: "New York", cqw: 1.9, weight: 400, lineHeight: 1.1 }
  card-headline:{ fontFamily: "New York", cqw: 2.0, weight: 400, lineHeight: 1.15 }
  stat-figure: { fontFamily: "New York", cqw: 3.0, weight: 400, lineHeight: 1.0 }
  quote-mark:  { fontFamily: "New York", cqw: 9.0, weight: 400, lineHeight: 0.5, color: "line" }
  h2:          { fontFamily: "New York", cqw: 4.0, weight: 400, lineHeight: 1.1 }
  h1:          { fontFamily: "New York", cqw: 6.2, weight: 400, lineHeight: 1.06 }
  display:     { fontFamily: "New York", cqw: 8.0, weight: 400, lineHeight: 1.04 }
...
- **Fonts** — use **New York** for display and **Abel** for body and labels; both match the captured storefront.
- **Weights** — use New York 400 and Abel 400/600 only.
- **Colors** — use the frontmatter hex values taken from the captured storefront; product photography supplies the leather-brown color.
...
The voice is a literary pairing: **Abel** at weight 400 (the thin-stroke didone, never
bold, always sentence case) carries every headline, numeral, and quote mark in ink; **Abel**
...
- **Abel 400** (ink, sentence case) for display; **Abel** body (gray) + labels (taupe, tracked).
...
Two ramps. The **reading ramp** (Abel body 1.0cqw gray, labels in px taupe) carries copy + chrome;
...
- **Playfair at 400, ink, sentence case** — never bold, never uppercase, never taupe. **Abel labels uppercase, 2–3px tracked, taupe.** Italic via Playfair italic for emphasis only.

## Depth & Surface

The flat plane is the only technique. Hierarchy from:

- **Type contrast** — Playfair serif vs Abel sans; the 8cqw→0.7cqw scale.
...
**Ground** `{colors.bg-primary}`, `pad-x`. **Composes** geo-ring (right, ~34cqw, solid+dashed), vertical-line (left), label, display/h1. **Focal** a 2–3 line Playfair `display`/`h1` headline in ink (italic on the key word), left-anchored, with a taupe `label` above and an Abel subtitle below. **Chrome** optional bottom meta row (Playfair value + taupe label). **Accent** the geo ring; optionally one `horizontal-accent` ink line (if it clears the meta). **Silence** ~55% empty. **Fixed** Playfair 400 ink sentence-case, ≤2 geo elements, 1px lines. **Free** title, ring placement, meta. **Density** sparse.
...
- **Abel + Abel via Google Fonts.** CJK pairing (Noto Serif SC 700/400) carries over; Playfair has no Hanzi italic — substitute weight/taupe for emphasis.
