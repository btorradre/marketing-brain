...
## Storyboards include the first asset set

User instruction, September 9, 2026: generate and inspect the talking-head presenter plus every distinct covering scene, overlay and first frame for each planned video insert as part of storyboard delivery. Use GPT Image 2 and attach the actual selected images to their corresponding cards. Keep character/product identity consistent and B-roll scenes visually distinct. Save/update the editing plan before generation. Text-only boards are interim work unless the user explicitly requests that limited scope. Preserve approved narration and keep competitor references in a separate lane.
