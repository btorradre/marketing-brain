...
payload={'text':text,'model_id':'eleven_v3','voice_settings':{'stability':0.5,'similarity_boost':0.85,'use_speaker_boost':True}}
(v/'tts-request.json').write_text(json.dumps(payload,indent=2)+'\n')
assert not (v/'narration-original.mp3').exists()
...
z=r.json();(v/'narration-original.mp3').write_bytes(base64.b64decode(z.pop('audio_base64')));(v/'alignment.json').write_text(json.dumps(z,indent=2)+'\n');print({'status':r.status_code,'bytes':(v/'narration-original.mp3').stat().st_size,'voice_id':d['voice_id']})
