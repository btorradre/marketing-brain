...
ads=json.loads((R/'resolve/manifest.json').read_text());start=int(sys.argv[1]) if len(sys.argv)>1 else 0;end=int(sys.argv[2]) if len(sys.argv)>2 else len(ads)
...
assert(pm:SaveProject()) assert(pm:ExportProject(p:GetName(),output.."/Eleanor-OldMoney-R4.drp"))
