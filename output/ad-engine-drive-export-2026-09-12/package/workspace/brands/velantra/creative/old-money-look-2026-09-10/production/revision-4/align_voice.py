...
for h in ['H1','H2','H3']:
 out=R/'resolve'/f'{h}-forced-alignment.json'
 if out.exists():continue
 text=json.loads((P/'voice'/h/'request.json').read_text())['text']
 with (P/'voice'/h/'narration.mp3').open('rb') as f:
  response=requests.post('https://api.elevenlabs.io/v1/forced-alignment',headers={'xi-api-key':env['ELEVENLABS_API_KEY']},data={'text':text},files={'file':('narration.mp3',f,'audio/mpeg')},timeout=180)
