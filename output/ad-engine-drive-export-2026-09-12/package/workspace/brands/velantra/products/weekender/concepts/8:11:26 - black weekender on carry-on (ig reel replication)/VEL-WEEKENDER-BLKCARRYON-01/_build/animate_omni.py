#!/usr/bin/env python3
"""Stage 3 — animate every picked still via Google Omni (Interactions API). BLACK build.
...
def motion_prompt(s):
    ...
    return (
        ...
        "anything. The bag's construction must not change at any point. The bag is ALL BLACK "
        "LEATHER over its entire exterior: no canvas, no woven texture, no cream or grey panel "
        "and no two tone split ever appears on it at any moment. Its interior stays smooth "
        "caramel tan leather. The handles stay smooth simple black leather tubes, the two short "
        "belt straps stay lying horizontally in their gold clasp plates and never swing loose, "
        "the hardware keeps its exact shape and stays warm brass gold on both sides, and no "
        "zipper, logo, engraving, lettering or brand stamp appears anywhere at any moment. "
