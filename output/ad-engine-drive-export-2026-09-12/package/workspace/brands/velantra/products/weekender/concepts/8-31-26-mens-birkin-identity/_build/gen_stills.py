...
RES = os.environ.get("GI2_RES", "1K")  # 2K rejects long prompts as of 9/01 — see BUILD-NOTES
...
def ref_path(key):
    return REFS[key]  # scenes.py stores absolute paths
...
def upload(key):
    ...
    out = subprocess.run([
        ...
        "-F", f"file=@{path}",
        "-F", "uploadPath=images/velantra-weekender-mensid",
