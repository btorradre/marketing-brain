...
CRE  = HERE.parent.parent / "8-31-26-greenscreen-everything" / "creators"
...
H    = {"X-Api-Key": KEY}
CREATORS = os.environ.get("CREATORS","A-diane").split(",")
...
VO_EXPECT = 57.84          # the take everything downstream is timed to

if __name__ == "__main__":
    import shutil
    _d = float(subprocess.run(["ffprobe","-v","error","-show_entries","format=duration",
                               "-of","csv=p=0",str(VO)],capture_output=True,text=True).stdout)
    assert abs(_d - VO_EXPECT) < 0.05, (
        f"VO is {_d:.2f}s but this build is timed to {VO_EXPECT}s. This script had its own hard-coded "
        f"VO path and got missed when the take was re-rendered, so three creators came back lipsynced "
        f"to the previous audio. Point VO at the current take before rendering.")
    ...
    for name in CREATORS:
        ...
        if not s.get("look_id"):
            d = post("https://api.heygen.com/v3/avatars",
                     {"type":"photo","name":f"viv-celeb-{name}",
