    ...
    for attempt in range(4):
        ...
        out = subprocess.run(
            ["curl", "-s", "-X", "POST", UPLOAD_URL,
             ...
             "-F", f"file=@{path}",
             "-F", "uploadPath=velantra-goeswith-vo",
...
def build_image_prompt(shot):
    pin = MANIFEST["flap_open_pin"] if shot["flap"] == "open" else MANIFEST["flap_closed_pin"]
    return " ".join([shot["keyframe_prompt"], MANIFEST["product_lock"], pin, MANIFEST["style_line"]])


def build_video_prompt(shot):
    imperf = MANIFEST["imperfections_product"] if shot["subject"] == "product" else MANIFEST["imperfections_human"]
    parts = [shot["motion_prompt"], MANIFEST["kling_flap_pin"]]
    if shot["flap"] == "open":
        parts.append("Contents lean out of the mouth behind the flap, never through it.")
    parts.append(imperf)
    return " ".join(parts)
...
def cmd_prompts(ids):
    for shot in shots_by_id(ids):
        print(f"===== {shot['id']} ({shot['time']}) refs={shot['refs']} flap={shot['flap']} =====")
        ...
        for sid in list(pending):
            ...
            if d.get("state") == "success":
                ...
                if rec["resultUrl"]:
                    ...
                    print(f"{sid} DONE -> {os.path.normpath(dest)}")
                del pending[sid]
...
def cmd_status():
    ...
    for kind in ("images", "videos"):
        ...
        for sid, rec in sorted(st[kind].items()):
            print(f" {sid}: {rec.get('state')} fail={rec.get('failMsg', '')}")
