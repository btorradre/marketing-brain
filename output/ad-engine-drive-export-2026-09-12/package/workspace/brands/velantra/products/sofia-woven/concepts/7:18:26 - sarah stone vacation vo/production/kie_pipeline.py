#!/usr/bin/env python3
"""VEL-STRAWTOTE-VACATION-VO-01 asset pipeline.
GPT Image 2 i2i keyframes -> Kling 3.0 i2v clips, all via kie.ai.

Usage:
  kie_pipeline.py upload
  kie_pipeline.py images [S01 S02 ...]     # default: all non-skip shots
  kie_pipeline.py poll-images
  kie_pipeline.py videos [S01 ...]         # needs poll-images done for those shots
  kie_pipeline.py poll-videos
  kie_pipeline.py status
"""
import json, os, ssl, sys, time, mimetypes, uuid, urllib.request, urllib.error
...
# shots whose bags are not (only) caramel — swap the caramel-specific lock wording
NEUTRAL_LOCK_IDS = {"S13", "S17b"}
    ...
    for attempt in range(4):
        ...
        out = subprocess.run(
            ["curl", "-s", "-X", "POST", UPLOAD_URL,
             ...
             "-F", f"file=@{path}",
             "-F", "uploadPath=velantra-vacation-vo",
...
def build_image_prompt(shot):
    parts = [shot["keyframe_prompt"]]
    if "tessa" in shot["refs"]:
        parts.append(MANIFEST["identity_line"])
    lock = MANIFEST["product_lock"]
    if shot["id"] in NEUTRAL_LOCK_IDS:
        lock = lock.replace("a smooth caramel leather flap", "a smooth leather flap")
        lock = lock.replace("two crossing caramel leather belt straps", "two crossing matching leather belt straps")
        lock = lock.replace("two rolled caramel leather top handles", "two rolled matching leather top handles")
    if shot["refs"]:
        parts.append(lock)
    parts.append(MANIFEST["style_line"])
    return " ".join(parts)

def shots_by_id(ids):
    all_shots = [s for s in MANIFEST["shots"] if not s.get("skip_generation")]
    if not ids:
        return all_shots
    return [s for s in all_shots if s["id"] in ids]
...
def cmd_images(ids):
    ...
    for shot in shots_by_id(ids):
        ...
        if r.get("code") != 200:
            print(f"{shot['id']} CREATE FAIL: {r}"); continue
    ...
    while pending and time.time() < deadline:
        for sid in list(pending):
            ...
            try:
                ...
                print(f"{sid} poll error {e}"); continue
            d = r.get("data") or {}
            ...
            if d.get("state") == "success":
                ...
                if rec["resultUrl"]:
                    dest = os.path.join(HERE, "..", "assets", out_dir, f"{sid}{ext}")
                    download(rec["resultUrl"], dest)
                    print(f"{sid} DONE ({rec['credits']}cr) -> {os.path.normpath(dest)}")
                del pending[sid]
...
def cmd_videos(ids):
    st = state()
    imperf = MANIFEST["kling_imperfections"]
    for shot in shots_by_id(ids):
        img = st["images"].get(shot["id"], {})
        if not img.get("resultUrl"):
            print(f"{shot['id']}: no keyframe yet, skipping"); continue
        prompt = f"{shot['motion_prompt']} {imperf}"
...
def cmd_status():
    ...
    for kind in ("images", "videos"):
        ...
        for sid, rec in sorted(st[kind].items()):
            print(f" {sid}: {rec.get('state')} credits={rec.get('credits')} fail={rec.get('failMsg','')}")

if __name__ == "__main__":
    load_key()
    cmd = sys.argv[1] if len(sys.argv) > 1 else "status"
    args = sys.argv[2:]
    if cmd == "upload": cmd_upload()
    elif cmd == "images": cmd_images(args)
    elif cmd == "poll-images": poll("images", "keyframes", ".png")
    elif cmd == "videos": cmd_videos(args)
    elif cmd == "poll-videos": poll("videos", "clips", ".mp4")
