...
for h in sys.argv[1:] or ['H1','H2','H3']:
 d=json.loads((O/f'{h}-pause-edit.json').read_text());p=O/(f'{h}-clean-master-resolve-r3.mov' if h=='H1' else f'{h}-clean-master-resolve-r2.mov')
 def pcm(f):
 ...
 x=pcm(O/f'{h}-source-48k.wav');expected=np.concatenate([x[r['source_start']*N:r['source_end']*N] for r in d['rows']]);got=pcm(p);report={'source_samples':len(x),'expected_samples':len(expected),'actual_samples':len(got),'duration_correct':len(got)==len(expected)}
 ...
 dest=O/f'deliverables/Motilli-V24-{h}-Natural-1.2x-clean.mp3';subprocess.run(['ffmpeg','-v','error','-y','-i',str(p),'-vn','-af','pan=mono|c0=0.5*c0+0.5*c1','-ar','44100','-codec:a','libmp3lame','-b:a','192k',str(dest)],check=True)
 wav=O/f'deliverables/Motilli-V24-{h}-Natural-1.2x-clean.wav';subprocess.run(['ffmpeg','-v','error','-y','-i',str(p),'-vn','-af','pan=mono|c0=0.5*c0+0.5*c1','-c:a','pcm_s24le',str(wav)],check=True)
