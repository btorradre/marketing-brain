...
def work(pair):
 h,t=pair;f=O/f'qa/{h}-window-{t}.mp3';subprocess.run(['ffmpeg','-v','error','-y','-ss',str(t),'-i',str(O/f'deliverables/Motilli-V25-{h}-Woman-Over-40-Natural-1.2x-clean.mp3'),'-t','12',str(f)],check=True)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ex:list(ex.map(work,[('H1',0),('H1',9),('H1',232)]))
