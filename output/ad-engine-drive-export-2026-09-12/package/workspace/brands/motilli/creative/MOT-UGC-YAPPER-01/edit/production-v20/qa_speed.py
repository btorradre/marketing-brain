...
for shift in np.arange(-.15,.1501,.01):
 ref=np.interp((ts+shift)*1.1,np.arange(len(a))*.01,a);c=float(np.corrcoef(ref,b)[0,1])
...
assert phrases(B/'deliverables/Motilli-Unbranded-VSL-v18.srt')==phrases(O/'deliverables/Motilli-Unbranded-VSL-v20-1.1x.srt')
ins=json.loads((B/'aligned-inserts.json').read_text());rows=[]
for i in ins:
 s=round(i['start_frame']/1.1);e=round(i['end_frame']/1.1);rows.append((i['id'],[max(0,s-1),s,(s+e)//2,e-1,e]))
rows.append(('Presenter and ending',[round(138/1.1),1500,4200,7800,8572]));frames=sorted({f for _,fs in rows for f in fs})
def grab(f):
...
report={'fps':30,'frames':8573,'duration_seconds':8573/30,'expected_unrounded_duration':9431/30/1.1,'audio_envelope_match_to_1_1x_source':best,'caption_phrases':227,'caption_words_unchanged':True,'review_frames':frames,'visual_review':'pending','native_speed':110,'native_pitch_correction':True};(Q/'speed-qa.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
