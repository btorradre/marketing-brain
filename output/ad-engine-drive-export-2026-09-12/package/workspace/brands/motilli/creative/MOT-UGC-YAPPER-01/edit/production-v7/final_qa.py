...
O=pathlib.Path(__file__).resolve().parent;V=O/'deliverables/Motilli-Unbranded-VSL-v7.mp4';Q=O/'qa/final';Q.mkdir(parents=True,exist_ok=True)
meta=json.loads(subprocess.check_output(['ffprobe','-v','error','-show_streams','-show_format','-of','json',str(V)]));(Q/'metadata.json').write_text(json.dumps(meta,indent=2));v=next(s for s in meta['streams'] if s['codec_type']=='video');assert (v['width'],v['height'])==(1080,1920);assert v['r_frame_rate']=='30/1';assert int(v['nb_frames'])==json.loads((O/'final-speech-ranges.json').read_text())['frames'];assert any(s['codec_type']=='audio' for s in meta['streams']);print('Final metadata verified',v['nb_frames'],meta['format']['duration'],flush=True)
ins=json.loads((O/'aligned-inserts.json').read_text());total=int(v['nb_frames']);frames={0,1,60,total-60,total-2,total-1}
for x in ins:frames.update([x['start_frame']-1,x['start_frame'],x['end_frame']-1,x['end_frame']])
speech_ranges=json.loads((O/'final-speech-ranges.json').read_text())['ranges']
for x in speech_ranges[1:]:frames.update([x['record_start']-1,x['record_start']])
...
# Inspect both consecutive frames at every linked presenter/audio pause cut.
for page in range(3):
 batch=speech_ranges[1:][page*5:(page+1)*5];im=Image.new('RGB',(600,5*440),'#eee');d=ImageDraw.Draw(im)
 for row,x in enumerate(batch):
  f=x['record_start'];d.text((8,row*440+4),f'Linked speech cut at {f/30:.3f}s | before / after',fill='black')
  for col,n in enumerate([f-1,f]):im.paste(ImageOps.contain(Image.open(Q/f'frame-{n:05d}.jpg'),(230,410)),(col*300,row*440+24))
 im.save(Q/f'speech-cuts-{page+1}.jpg')
# Probe the first and final spoken frames, with no ending hold.

fs=[0,1,60,total-60,total-2,total-1];im=Image.new('RGB',(1200,380),'#eee');d=ImageDraw.Draw(im)
...
with (Q/'black-silence-scan.txt').open('w') as out:subprocess.run(['ffmpeg','-hide_banner','-i',str(V),'-vf','blackdetect=d=0.08:pix_th=0.06','-af','silencedetect=noise=-40dB:d=0.35','-f','null','-'],stderr=out,stdout=subprocess.DEVNULL,check=True)
