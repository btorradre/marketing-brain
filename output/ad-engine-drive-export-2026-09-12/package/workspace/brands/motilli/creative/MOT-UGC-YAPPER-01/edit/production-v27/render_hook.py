...
O=Path(__file__).resolve().parent;ROOT=O.parents[5];key=next(l.split('=',1)[1].strip().strip('"').strip("'") for l in (ROOT/'.env').read_text().splitlines() if l.startswith('HEYGEN_API_KEY='));look='3c421c1dc94c35b2f46a50496bbd781c';h=sys.argv[1];S=O/(h+'-hook');S.mkdir(exist_ok=True)
with httpx.Client(timeout=180,headers={'X-Api-Key':key}) as c:
 ...
 assert (S/'audio-ready.json').exists()
 audio=S/'hook-with-context.mp3';dest=S/'presenter-avatar-v-original.mp4';jf=S/'heygen-video-job.json'
 if jf.exists():job=json.loads(jf.read_text())
  ...
  payload={'type':'avatar','avatar_id':look,'audio_asset_id':aid,'engine':{'type':'avatar_v'},'aspect_ratio':'9:16','resolution':'1080p','title':f'Motilli V27 {h} Woman Over 40 Natural Resolve 1.1x hook'};save('heygen-video-request.json',payload)
