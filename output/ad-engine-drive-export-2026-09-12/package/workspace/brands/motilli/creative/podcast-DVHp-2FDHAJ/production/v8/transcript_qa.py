...
V=P/'deliverables/Motilli-Podcast-No-Dead-Space.mp4';out=P/'qa/final-dialogue.mp3'
...
assert r.status_code==200,(r.status_code,r.text[:300]);data=r.json();(P/'qa/final-scribe.json').write_text(json.dumps(data,indent=2));script=' '.join(t['text'] for t in json.loads((P.parent/'v5/aligned-turns.json').read_text()))
