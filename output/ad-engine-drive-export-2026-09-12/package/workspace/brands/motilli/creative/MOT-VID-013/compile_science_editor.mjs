...
const output=path.join(here,'output/science-v2/editor');
...
for(const variant of status.variants){
 ...
 let project=createProject({id:variant.id,name:variant.id+' — science revision',width:1080,height:1920,fps:30,assets,meta:{status:status.status,sourceBoard:'mot-vid-013-science-v2',narrationSpeed:vo.speed}});
 ...
 const labels=[
  ...
  {id:'fiber',text:'LOW-VISCOSITY\nSOLUBLE FIBER',start:find('S22').t,end:find('S22').t_end},
