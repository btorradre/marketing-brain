...
import requests
p=Path(__file__).resolve().parent;root=p.parents[5];src=p/'exports/MOT-VID-013-R2-A.mp4'
a=p/'export-A-listening-audio.m4a';subprocess.run(['ffmpeg','-v','error','-y','-i',str(src),'-vn','-c:a','aac','-b:a','96k',str(a)],check=True)
key=next(l.split('=',1)[1].strip().strip('"').strip("'") for l in (root/'.env').read_text().splitlines() if l.startswith('GEMINI_API_KEY='))
prompt='Listen to the entire exported ad. This revision uses original speech at 1.1x and removes 31 long pauses with short quiet pads. Review actual audio, especially intelligibility, dead spots and missing breaths, clipped phonemes at edits, abrupt edits, vocal pitch artifacts, distracting music, pauses, exact words, pronunciation of Motilli, apigenin and chlorophyllin. Do not infer from script; compare the actual sound. Return JSON with verdict(pass/revise), voice_description, pace_and_intelligibility, music_description_and_balance, pronunciation, omitted_words, extra_words, audible_issues[{time_s,issue,severity}], last_spoken_word_time_s, whether_end_fade_is_clean. Approved script: '+(p.parent.parent/'narration-script.txt').read_text()
