...
for pr in projects:
 ...
 page+=f'<article><video controls playsinline preload="metadata" poster="{v}-poster.jpg" src="{name}"></video><h2>{v} · {html.escape(pr["title"])}</h2><p>95.9 seconds · 1080 × 1920</p><a href="{name}" download>Download full ad {v}</a></article>'
shutil.copy2(p.parent/'capcut-r2/MOT-VID-013-R2.srt',dest/'MOT-VID-013-captions.srt');page+='</section><footer><a href="MOT-VID-013-captions.srt" download>Download captions</a></footer></main><script>document.querySelectorAll("video").forEach(v=>v.addEventListener("play",()=>document.querySelectorAll("video").forEach(o=>{if(o!==v)o.pause()})))</script></html>'
(dest/'review.html').write_text(page);url='http://localhost:8765/assets/mot-vid-013-hook-variations/review.html';r=requests.get(url,timeout=10);r.encoding='utf-8';assert r.status_code==200 and 'All three hook variations' in r.text
for pr in projects:
 with requests.get(url.rsplit('/',1)[0]+f'/MOT-VID-013-Hook-{pr["variant"]}.mp4',stream=True,timeout=10) as r:assert r.status_code==200
print(url)
