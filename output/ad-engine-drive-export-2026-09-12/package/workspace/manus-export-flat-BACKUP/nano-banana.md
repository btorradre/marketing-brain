...
Reference for generating AI images with the "nano-banana" command-line tool, which wraps Google's Gemini image models (Gemini 3.1 Flash Image Preview by default, Gemini 3 Pro available). Use this whenever the task is to generate an image, sprite, mockup, or any marketing/game/video visual asset and you have (or can set up) this CLI and a Gemini API key. It handles multi-resolution output (512px to 4K), aspect ratio control, reference-image-driven editing and style transfer, a green-screen workflow for producing transparent assets, cost tracking, and exact-dimension control.
...
### Cost tracking
Every generation is logged to a local cost file. View a running summary at any time with:
```bash
nano-banana --costs
```
