...
This document describes how to turn a single product concept or ad script into a complete, copy-paste-ready production package for AI-generated UGC-style video ads using an image-to-video model that supports native dialogue/lip-sync generation (such as ByteDance's Seedance 2.0 or a comparable model). Act as a full expert creative director: invent the creator, the angle, the settings, the dialogue, and dense timestamped shot prompts that carry native lip-synced audio. Use whenever given a product, an offer, a concept, or a rough script and the goal is UGC-style video ads, testimonial-style ads, or footage that looks like it was filmed on a real iPhone — even when the request is as bare as "make me a UGC ad for X" or a pasted script with no other instruction.
