---
name: broll-auditor
description: Quality-gate process for auditing AI-generated B-roll assets (images and videos) after any image-to-image or video generation pipeline has run. Compares every generated asset against the reference frame it was supposed to be based on and against real product reference photos, to catch the most common and costly generation failures before assets reach an editor. Use this immediately after any pipeline that generates B-roll via image-to-image or video generation for an ad — before those assets are handed off for editing.
---

# B-Roll Auditor

This is a post-generation quality gate, not a generation pipeline itself. It runs strictly **after** generation, never during.
...
For each generated asset, compare it side-by-side against: (1) its paired reference frame, and (2) the product reference photos. Use a capable multimodal vision model for this comparison — feed it all the relevant images plus the checklist below, and have it return a structured verdict. Score every item **PASS**, **WARN**, or **FAIL**, with a written note explaining the score. Use a low, strict temperature setting for consistent judgments, and process assets one at a time rather than in parallel so each judgment stays independent.
