...
Guide from idea to merged PR for a new registry block or component.
...
Decide what's being built. The registry has two item types:
...
- **Internal engineering-org contributors:** run the project's own docs-image upload script (requires the appropriate cloud credentials).
...
`{kind}` is `blocks` or `components` depending on what was built in Step 1.
...
# 5. Publish so reviewers can preview
npx hyperframes publish
...
# 8. Push and open PR with the preview link
git push origin feat/registry-{name}
gh pr create --title "feat(registry): {name}" --body "preview: {hosted-preview-url}"
```

**If there's no GitHub account:** one is needed to open a PR. Sign up, then run `gh auth login`.
...
- [ ] `npx hyperframes publish` run (claim the project URL)
