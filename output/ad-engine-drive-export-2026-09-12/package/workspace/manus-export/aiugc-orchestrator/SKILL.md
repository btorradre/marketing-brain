...
description: End-to-end AI UGC video ad production. Takes a script and a brand, generates a voiceover, creates an avatar image, produces a lip-synced talking-head video, generates matching B-roll, and stitches a finished, polished video. Optionally starts from a reference video for full scene-structure replication. Use when the user wants to produce a complete AI UGC video ad from scratch in one continuous production flow, rather than running each production step separately.
---

# AI UGC Orchestrator — End-to-End AI UGC Video Ad Production

Takes a script and a brand and produces a finished AI UGC (user-generated-content-style) video ad: a generated voiceover, an avatar image, a lip-synced talking-head video, optional matching B-roll, and a finished stitched-and-polished video — all in one continuous flow rather than as separate manual steps.

## Golden Nugget Doctrine (apply first)

Before any hook, angle, script, or concept decision is made, name the golden nugget: the single most emotionally loaded deep frame in the research or reference — the real motive that makes buyers act, never the surface theme. Topic is not motive: "memory loss" is a topic; "I thought I was getting dementia just like my mum did" is the frame. Surface angles buy mild curiosity; deep frames trigger identification so strong the viewer feels caught. State the golden nugget in one explicit sentence before producing anything. If it doesn't surface from the given script/brand material, mine the brand's reviews/VoC until it does — never default to a surface angle.

## Two non-negotiable rules

### Rule 1 — Reference-video analysis always goes first

Whenever a reference video is provided, the very first step — before any generation happens — is a full frame-by-frame analysis of that reference video using a vision-capable model. Nothing gets generated until this analysis is complete and saved. This analysis is the source of truth for every downstream step: it tells you exactly what to generate, at what timestamps, in what style.
...
3. Transform it with a prompt describing the adapted description for the new brand/mechanism, at a moderate transformation strength (roughly 0.6) so composition and rhythm carry over while content updates.
...
[Stage 6] Post-processing
    ...
    v
[Stage 7] Deliver the final asset
```

## Required inputs

1. **Script** — the ad script text.
2. **Brand name** — access that brand's voice/research documents so downstream steps reflect the real avatar, mechanism, and product.
...
5. **Voice** — a specific voice to use for text-to-speech (or a cloned voice from the brand's existing voice library).
...
9. **Resolution** — a standard resolution default, or a lower one for a faster/cheaper draft.
...
- Choose a TTS model/mode documented as clearest for the target language — avoid a general multilingual mode when a dedicated, crisper English-only mode exists; the multilingual modes tend to sound muffled or "underwater."
- If no premium TTS is available, fall back to whatever built-in TTS the talking-head/lip-sync model itself offers, but treat that as a lower-quality fallback, not the default choice.
