...
description: Takes any TikTok URL and recreates it as a short POV (point-of-view) video ad for a Velantra product (Boat Tote, Weekender, or Meridian), preserving the original clip's POV hook text overlay and its original music while swapping in the product. Use when the user wants to replicate a TikTok POV video for Velantra, or says "replicate this TikTok for Velantra", "POV TikTok replicator", or "make a Velantra POV from this link".
---

# Velantra POV TikTok Replicator

Use this to take any TikTok URL and recreate it as a short POV (point-of-view) video ad for a Velantra product — the Boat Tote, Weekender, or Meridian. It produces a finished vertical video that preserves the original clip's POV hook text overlay and its original music, with the product swapped in.
...
2. **Target product** (optional, default to whichever the brief implies) — Meridian, Boat Tote, or Weekender.
3. **Colorway** (optional — if omitted, choose one based on the scene analysis's color/lighting data).
4. **Output location** (optional).
