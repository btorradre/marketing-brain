#!/usr/bin/env bash
# Stitch Cut 1 + Cut 2 with a hard cut into one finished 9:16 ad.
#
# Cut 1 is assumed to carry native voiceover audio. Cut 2 (a literal/physical
# image-to-video product-action clip) frequently ships silent — if so, this
# script synthesizes a silent stereo audio track for Cut 2 first so the
# concatenation doesn't desync the audio/video streams.
#
# Requires: ffmpeg, ffprobe (both part of a standard ffmpeg install).
#
...
# Normalize both clips to the same resolution/framerate/audio format, then
# concat. Re-encode is cheap and avoids container-level mismatch surprises.
ffmpeg -y -hide_banner -loglevel error \
  -i "$C1" -i "$C2_PREP" \
