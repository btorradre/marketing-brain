---
name: video-gen
description: Turn still images into short video clips using Google's Veo 3 (image-to-video) through the Gemini API. Use whenever the user wants to animate an image, generate video from a photo or product still, create B-roll or motion clips from frames, build image-to-video, animate a frame sequence into clips, or says "Veo 3", "animate this", "make this image move", "turn these frames into video", or "image to video" — even if they don't name the model. Also use it for the second half of a frames-first workflow where stills were generated and now each needs to be animated into a clip.
---
...
After it finishes, report the saved clip paths and share the resulting MP4s with the user for review.
