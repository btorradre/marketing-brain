...
- `Unknown params: <name>` — schema doesn't accept this flag. Run `higgsfield model get <model_id>` and check.
...
If `Failed to decode response. Body: <html>...captcha-delivery...` appears, the server's anti-bot fired. Wait 30s and retry. If persistent, this needs escalation on the Higgsfield side.

## Cost

`higgsfield generate cost <model_id> ...` returns a credit estimate without submitting. Useful when the user asks "how much will this cost?".
