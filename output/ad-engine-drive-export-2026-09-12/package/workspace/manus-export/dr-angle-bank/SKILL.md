...
This skill is one component of the larger Direct Response OS. Before writing anything, load and internalize the twelve laws and the full angle-record schema from the direct-response-os skill's reference material — the schema is the contract, and no record here should deviate from it.
...
2. **Every angle is sourced.** Each entry references exactly where it came from: a review, a forum thread, a comment section, survey free-text, or ad-account data. Unsourced means it doesn't enter the bank.
...
| **Six-month shelf life** | Does the idea survive a calendar change? | `shelf_life` | `dated` is a rejection. Rewrite around a durable reason to believe, or drop it. |
| **Swap test** | Drop a competitor's product in. Does it still read perfectly? | `swap_test` | `fail` caps the record at MEDIUM priority and blocks it from being briefed until it's rebuilt around the product's own specific facts. |
