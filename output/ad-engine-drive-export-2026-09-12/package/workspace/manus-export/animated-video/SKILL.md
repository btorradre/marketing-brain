---
name: animated-video
description: End-to-end animated video ad pipeline (3D animated, motion graphics, anthropomorphic-character style) using a video-generation model with NATIVE audio generation, so no separate voiceover recording step is needed. Takes a script + brief + brand, breaks it into a scene-beat shot plan, generates still keyframes via image-to-image with product references, animates each still with native audio enabled (dialogue/SFX/ambient baked directly into each clip), stitches clips with hard cuts, and optionally adds word-synced captions in a polish pass. Use when the user wants an animated (3D animated, motion graphics, anthropomorphic-character) video ad and wants the video model to handle audio generation natively rather than dubbing a separately recorded voiceover over it.
---

# Animated Video — 3D/Motion-Graphics Ad Pipeline (Native Audio)

Production pipeline for animated-style direct-response video ads (3D animated, anthropomorphic characters, motion graphics, "inside the body" mechanism diagrams) built using an image-to-video model that supports native audio generation — dialogue, SFX, and ambient sound baked directly into each clip, with no separate voiceover recording needed.

This is a sibling process to a claymation/stop-motion pipeline — same architecture, different visual style, and a different audio strategy (see below).

**When NOT to use this:**
- Stop-motion/claymation style → use a claymation-specific process instead (same architecture, but voiceover comes from a separate TTS engine and clips are silent until stitched).
- Live-action replication → use a live-action video-replication process.
- Talking-head/UGC formats → use a UGC-specific pipeline instead.
- If audio must come from a separately recorded/cloned voiceover for every segment of a multi-segment animated ad (rather than the video model's own native voice) → see the "voice consistency" rule below; the native-audio approach in this document assumes short, single- or few-segment dialogue runs per character.

## Golden Nugget Doctrine (apply first)

Before any hook, angle, script, or concept decision is made, name the golden nugget: the single most emotionally loaded deep frame in the research — the real motive that makes buyers act, never the surface theme. Topic ≠ motive — dig one layer deeper than the surface theme. State the golden nugget in one explicit sentence before drafting. If it doesn't surface from what's given, mine the brand's reviews/VoC until it does — never default to a surface angle.
...
2. **Lock the start frame = end frame on every generation call.** Pass the same keyframe image as both the start and end reference. This forces the model to return the camera/composition to the starting frame by clip-end, killing zoom drift, push-in/pull-back twitches, and background-element drift.
...
6. **Use one canonical character keyframe.** Generate the first shot once. Regenerate all subsequent keyframes using that first shot as the PRIMARY reference at a low creative-variance setting (roughly 0.28-0.35 on a 0-1 temperature/variance scale), with an explicit character-lock description (hair style, distinguishing accessory, correct number of fingers, etc.). Don't let generation variance rebuild the character from scratch shot to shot.
7. **Phonetically spell out difficult words for text-to-speech.** Some words get consistently mispronounced by both video-native audio and separate TTS engines (e.g. "constipation" → "con-stuh-pay-shun"). Spell them out phonetically in the spoken text. Leave ellipses in for natural pauses.
8. **Use real video stabilization, not a basic deshake filter.** A basic deshake filter cannot fully counter the camera drift these models introduce. Use a stabilization tool with two-pass motion detection and transform (e.g. ffmpeg's vidstab plugin — `vidstabdetect` + `vidstabtransform`), or fall back to feature-matching stabilization (e.g. via OpenCV) if that's unavailable.
...
```
{
  ...
  "reference_images": [keyframe_image, keyframe_image],   # same image twice = start/end lock
...
├── images/shot_NN.png
├── clips/shot_NN.mp4          # already has native audio baked in
├── final/final.mp4            # simple concat, no separate VO mix
└── polish/ (optional, captioned final)
```

No separate voiceover audio directory is needed — audio lives in the clips themselves.
