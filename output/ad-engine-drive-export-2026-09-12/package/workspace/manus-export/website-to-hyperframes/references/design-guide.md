...
5-10 key colors with HEX values from the extracted design-token data and their roles:
...
Name every notable UI component you see in the screenshots. Be specific:
...
- Use **exact HEX values** from the extracted design-token data. Do not approximate.
- Name components by what you see in the screenshots, not generic terms.
...
- No "Assets" section — the extracted asset-description index already covers this.
- No "Motion" section — the storyboard specifies motion per-beat.

## Worked example (dark, cinematic brand)
