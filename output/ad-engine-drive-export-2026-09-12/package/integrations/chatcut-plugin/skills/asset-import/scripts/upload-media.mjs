...
async function prepareTranscriptionAudioFile(input, metadata, context) {
  ...
  return {
    ...audio,
    sampleBase64: await prepareTranscriptionLanguageSample(audio, context),
  };
}

export async function prepareTranscriptionLanguageSample(audio, context) {
  const path = join(
    context.workDir,
    `chatcut-${context.nameNoExt}-language-${process.pid}-${context.index}.ogg`,
  );
  // Bound client work and request size; language detection needs only a short
  // sample, including when the full transcription audio is several hours long.
  await run(
    context.options.ffmpeg,
    [
      "-nostdin",
  ...
  if (!sample.length || sample.length > 256 * 1024) {
    fail("Could not prepare a bounded language detection sample.");
  }
  return sample.toString("base64");
}

export function parseTranscriptionWaveformMetadata(metadata) {
  const peaks = [];
  let samples = 0;
  for (const frame of metadata.split(/frame:/).slice(1)) {
    const peak = frame.match(
      /Overall\.Peak_level=(-?(?:\d+(?:\.\d+)?|inf))/,
    )?.[1];
    const count = Number(
      frame.match(/Overall\.Number_of_samples=(\d+(?:\.\d+)?)/)?.[1],
    );
    if (
      peak === undefined ||
      ...
      throw new Error("Invalid transcription waveform frame");
    }
    const db = Number(peak);
    peaks.push(
      Number.isFinite(db) ? Math.round(Math.min(1, 10 ** (db / 20)) * 255) : 0,
    );
    samples += count;
  }
  if (peaks.length === 0 || peaks.length !== Math.ceil(samples / 480)) {
    throw new Error("Incomplete transcription waveform");
  }
  return { durationSeconds: samples / 48_000, peaks };
}

export async function prepareTranscriptionWaveform(
  audio,
  durationSeconds,
  context,
) {
  ...
  const decoded = parseTranscriptionWaveformMetadata(
    await readFile(metadataPath, "utf8"),
  );
  const { peaks } = decoded;
  // Container duration can differ from decoded AAC/Opus samples due to codec
  // padding. Waveform timing must describe the actual audio sent to ASR.
  durationSeconds = decoded.durationSeconds;
  if (durationSeconds > TRANSCRIPTION_WAVEFORM_MAX_SECONDS) return undefined;
...
async function prepareTranscriptionAudio(input, metadata, assetType, context) {
  ...
  return {
    ...
    audioDurationSeconds:
      waveform?.durationSeconds ?? metadata.audioDuration ?? metadata.duration,
    transcriptionAudio,
    waveform,
  };
}
  ...
  return postJson(
    {
      finalizeTranscriptionAudioRequest: {
        ...
        assetId,
        assetType,
        sampleBase64: transcriptionAudio.sampleBase64,
