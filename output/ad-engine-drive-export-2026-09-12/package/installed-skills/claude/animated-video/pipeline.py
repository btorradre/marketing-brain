...
=======================
Script + brief + brand → animated-style video ad.
Kling 3.0 generates audio natively (sound=True) — no ElevenLabs.

Usage:
    python3 pipeline.py --brand Motilli --concept GG-ANIM-01 \
        --script ./script.md \
        --brief "inside-the-gut mechanism with anthropomorphic bacteria characters"
...
Differs from the `claymation` skill in three ways:
    1. Visual style block = ANIMATED_STYLE_BLOCK (3D animated, not clay)
    2. Kling payload uses sound=True and bakes dialogue + SFX into the prompt
    3. Stage 4 (ElevenLabs TTS) is skipped — audio lives in each Kling clip
"""
...
ELEVENLABS_API_KEY = os.environ.get("ELEVENLABS_API_KEY", "")  # not required for animated-video
...
# ---------------------------------------------------------------------------
CLAY_STYLE_BLOCK = """
ANIMATED STYLE (MANDATORY, enforce in every frame):
- Polished 3D animated aesthetic — stylized but clean, cinematic lighting
- Soft rim lighting, hand-painted textures, subtle subsurface scattering
- Anthropomorphic characters with expressive, exaggerated features
- Warm palette with selective brand-color accents (never oversaturated)
- Shallow depth of field, cinematic composition
- 9:16 vertical, macro framing on character-focused shots
- Stylized — NOT photorealistic, NOT flat 2D cartoon, NOT clay, NOT technical diagram
"""
...
def animate_shots(shot_plan, image_results, output_dir):
    ...
    for shot, ir in zip(shot_plan["shots"], image_results):
        ...
        # Animated motion + organic cues + Kling native audio directives (dialogue + SFX)
        motion = shot.get("motion_prompt", "Smooth animated motion.")
        dialogue = (shot.get("dialogue_line") or "").strip()
        voice_style = shot.get("voice_style_hint", "warm conversational narrator")
        sfx = (shot.get("sfx_direction") or "").strip()
        ...
        audio_parts = []
        if dialogue:
            audio_parts.append(f'Narration voice ({voice_style}) speaks: "{dialogue}"')
        if sfx:
            audio_parts.append(f"Ambient audio: {sfx}")
        audio_block = " ".join(audio_parts)

        kling_prompt = (
            f"Polished animated 3D video with cinematic lighting. {motion} "
            f"{ORGANIC_CUES} {audio_block}"
        ).strip()
        ...
        payload = {
            ...
            "input": {
                ...
                "sound": True,   # Kling generates dialogue/SFX natively
...
def main():
    ...
    if not re.match(r"^[A-Z]{2,3}-ANIM-\d{2}$", args.concept):
        log("Main", f"WARN: concept '{args.concept}' does not match ANGLE-ANIM-## convention")
    ...
    # --- Stage 4: SKIPPED (Kling clips include native audio) ---
    log(4, "Skipped — Kling generated audio natively per-clip")
    progress.mark("stage4_vo_skipped")

    # --- Stage 5: Stitch (simple concat since each clip already has synced audio) ---
    log(5, "FFMPEG — concat clips (audio baked in)")
    final_dir = os.path.join(output_dir, "final")
    os.makedirs(final_dir, exist_ok=True)
    final_path = os.path.join(final_dir, "final.mp4")
    concat_list = os.path.join(final_dir, "_concat.txt")
    clip_paths = [cr["path"] for cr in clip_results if cr.get("status") in ("success", "cached") and cr.get("path")]
    if not clip_paths:
        raise RuntimeError("No Kling clips available to stitch")
    with open(concat_list, "w") as f:
        for p in clip_paths:
            safe = p.replace("'", "'\\''")
            f.write(f"file '{safe}'\n")
    # Re-encode for uniform codec (Kling clips vary)
    subprocess.run(
        ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_list,
         "-c:v", "libx264", "-preset", "medium", "-crf", "20",
         "-c:a", "aac", "-b:a", "192k",
